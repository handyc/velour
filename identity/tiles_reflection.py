...
def generate_tileset_from_identity(force_name=None,
    ...
    tileset = TileSet.objects.create(
        ...
        source_metadata={
            ...
            'snapshot_key':   key,
            # If this tileset was caused by a meditation bouncing,
            # record the meditation so we can trace the hop AND so
            # that we never bounce back to create a second-level
            # meditation about a tileset that was already about a
            # meditation. The loop exits after one hop.
