#!/bin/bash
# adminsetup.sh — privileged provisioning for {{ app_label }}
#
...
#   2. Create the project user ({{ deploy_user }}) if missing, lock down $HOME.
#   3. Create /var/www/webapps/{{ deploy_user }}/{run,static,log} with correct
#      ownership.
#   4. Rsync the staging tree into /home/{{ deploy_user }}/ and chown it.
