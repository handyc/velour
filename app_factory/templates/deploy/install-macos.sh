#!/bin/bash
# install-macos.sh — first-time install for {{ app_label }} on macOS.
#
...
echo "=========================================="
echo "  {{ app_label }} — install-macos.sh"
echo "  Home: $APP_HOME"
echo "  User: $(whoami)"
echo "=========================================="
...
# Optional: write a LaunchAgent so {{ app_label }} auto-starts at login.
if [ "$WANT_LAUNCHD" = "1" ]; then
    PLIST_DIR="$HOME/Library/LaunchAgents"
    PLIST_PATH="$PLIST_DIR/com.{{ project_name }}.dev.plist"
    mkdir -p "$PLIST_DIR"
    cat > "$PLIST_PATH" <<EOF
...
<plist version="1.0">
<dict>
    <key>Label</key>          <string>com.{{ project_name }}.dev</string>
    ...
    <key>StandardOutPath</key><string>/tmp/{{ project_name }}_runserver.log</string>
    <key>StandardErrorPath</key><string>/tmp/{{ project_name }}_runserver.log</string>
</dict>
</plist>
EOF
