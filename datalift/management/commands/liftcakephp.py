"""Lift a CakePHP application into Django.
...
Reads `<app>/config/routes.php` and `<app>/src/Controller/**/*.php`
(including prefixed subdirs like `src/Controller/Admin/...`) and
emits `<app>/urls_cakephp.py` + `<app>/views_cakephp.py`.

See :mod:`datalift.cakephp_lifter`.
"""
...
from datalift.cakephp_lifter import apply, parse_cakephp, render_worklist


class Command(BaseCommand):
    help = 'Lift a CakePHP application into Django.'

    def add_arguments(self, parser):
        parser.add_argument('source', help='Path to the CakePHP app root.')
    ...
    def handle(self, *args, **opts):
        ...
        result = parse_cakephp(source)
        worklist_path = (
            Path(opts['worklist']).resolve() if opts['worklist']
            else project_root / 'liftcakephp_worklist.md'
        )
