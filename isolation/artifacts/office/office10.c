/* office10.c — Win95-style 13-app suite. Linux x86_64. No libc.
 *
 *   shell  notepad  word  mail  sheet  paint  hex  bfc  files
 *   find  calc  mines  ask   garden
 *
 * Same apps as office9.  One garden display fix:
 *
 * garden_render_grid clamped thumb_w to `cols / 8` but then bumped
 * it back to a hard-coded "MVP minimum 10" — undoing the clamp on
 * any terminal narrower than 80 columns.  At 75 cols the grid
 * computed 8 × 10 = 80, overflowing the visible viewport by 5 cols.
 * The right edge of every thumbnail row spilled onto the row below,
 * but only the last (gy=7) row's spill survived because nothing else
 * painted on top of it, leaving 5 stray status-border chars on line
 * 25 of every garden render.  Fix: lower the floor to 6 (the
 * smallest thumb_w where the title + status rows still fit) and
 * drop the post-clamp bump entirely.
 *
 * Carried forward from office9: garden V key — cursor on a thumb +
 * V launches a real jailed office10 with that genome loaded, drops
 * the user into the suite shell, q tears down the jail and returns
 * to garden.  Files saved during V mode live inside the jail dir
 * and vanish on exit, so V is non-destructive — purely for "what
 * does this colour scheme feel like in real use" exploration.
 */
...
static void garden_render_grid(int cursor, int cols, int rows) {
    ...
    /* 8×8 thumbs at cols/8 × rows/8.  Floor at 6 cols / 3 rows: 6 is
     * the smallest thumb_w where garden_render_thumb's title bar
     * (" O7" + slots + "_X ") still fits exactly without overflow,
     * and the menu bar's narrow form (" FEVH") covers it too.
     * Sub-floor terminals (cols < 48 or rows < 24) accept some
     * overflow rather than silently rendering a partial grid. */
    int thumb_w = cols / 8;
    int thumb_h = rows / 8;
