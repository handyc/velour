/* office44.c — Win95-style 18-app suite. Linux x86_64. No libc.
 *
 *   shell  notepad  word  mail  sheet  paint  hex  bfc  files
 *   find  calc  mines  ask   garden  hxhnt  rpg  lsys  bytebeat
 *   saver
 *
 * office44 fixes the shell home-screen menu.  run_shell prints its
 ...
 *     into /tmp/office44_keys.txt, scan for "sk-" tokens, and
 ...
 * 8000 Hz u8 mono into /tmp/office44_bb.raw and pipe to aplay via
...
static void show_about(const char *title) {
    ...
    body_at(2, 6, "  find calc mines ask garden hxhnt rpg lsys bytebeat saver", SCREEN_W - 4);
...
static int run_shell(int argc, char **argv) {
    ...
    while (running) {
        ...
        body_at(2, 4, "  notepad  word  mail  sheet  paint  hex  bfc  files",
                SCREEN_W - 4);
        body_at(2, 5, "  find  calc  mines  ask  garden  hxhnt  rpg  lsys",
                SCREEN_W - 4);
        body_at(2, 6, "  bytebeat  saver  exit",
                SCREEN_W - 4);
        body_at(2, 7, "  (Alt+F / F10 opens menus in every app)", SCREEN_W - 4);
        if (msg[0]) {
            sgrbgfg(COL_BAR_BG, msg_kind == 2 ? 88 : 22);
            body_at(2, 8, msg, SCREEN_W - 4);
            sgrbgfg(COL_BAR_BG, COL_BAR_FG);
        }
        cup(2, cur_y + 7);
        sgrbgfg(15, 0);
        ...
        if (k[0] == '\r' || k[0] == '\n') {
            ...
            else if (scmp(cmd, "saver") == 0 || scmp(cmd, "screensaver") == 0)
                                                rc = run_screensaver(sub_argc, sub_argv);
...
/* office44 — preseed the API key with a random `sk-…` token from
 * https://github.com/dan1471/FREE-openai-api-keys (README.md).  Curl
 * downloads to /tmp, then a one-pass reservoir sample picks one of
 * the matching tokens uniformly at random.  Keys in that repo are
 * mostly placeholder/demo strings; this is a convenience for toy
 * dev workflows against open-proxy endpoints, not production use. */
#define ASK_KEYS_FILE  "/tmp/" APP_NAME "_keys.txt"
#define ASK_KEYS_URL   "https://raw.githubusercontent.com/dan1471/FREE-openai-api-keys/main/README.md"
#define ASK_KEYS_BUF   65536
...
static void ask_fetch_random_key(void) {
    ...
    for (long i = 0; i < m - 2; i++) {
        if (ask_keys_buf[i] == 's' && ask_keys_buf[i+1] == 'k'
                                   && ask_keys_buf[i+2] == '-') {
            long j = i + 3;
            while (j < m) {
                char ch = ask_keys_buf[j];
                int ok = (ch >= 'a' && ch <= 'z') ||
                         (ch >= 'A' && ch <= 'Z') ||
                         (ch >= '0' && ch <= '9') ||
                         ch == '-' || ch == '_';
...
static void ask_settings_modal(void) {
    ...
    while (1) {
        ...
        body_at(2, 4, "Up/Down select; ENTER edit; r=random key; ESC save+close.",
                SCREEN_W - 4);
        for (int i = 0; i < 3; i++) {
            cup(2, 6 + i * 2);
            sgrbgfg(COL_BAR_BG, COL_BAR_FG);
        ...
        if (!editing) {
             ...
             * bytes in one read; pre-office44 only the first was
...
#define RPG_MAP_H       64
/* office44 — 3×3 mosaic of overworlds.  The world-cell arrays cover
...
/* office44 — meta-overworld coordinate stack.  rpg_world_pos[0] is
...
/* office44 NPC layer.  rpg_npc_at[i] = 0 → no NPC; otherwise the byte
...
/* office44 wander layer.  Each animal/NPC has a current step index
...
/* office44 — when set, rpg_render_view drops origin_y to 0 and uses
...
static void rpg_init_map(const unsigned char *ruleset) {
    /* office44 — populate the 192×192 mosaic by running 9 independent
...
static void rpg_init_entities(int spawn_x, int spawn_y) {
    ...
    /* office44 — seed entities per-sub-overworld so each chunk's
...
/* office44 — generate a procedural closed-loop path of hex directions.
...
/* ── saver: rpg screensaver (office44) ────────────────────
...
/* ── bytebeat: tiny PCM synth (office44) ─────────────────
