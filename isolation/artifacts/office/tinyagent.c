...
int main_c(int argc, char **argv, char **envp) {
    ...
    /* tinyagent: dropped from officecoder's main_c — ~600 B savings.
     *   - hx_active_init: hxhnt palette/genome bootstrap, rpg-only
     *   - gd_embedded chrome decode: relied on garden state, never used
     *   - uname(2)-based "garden-XXXXXXXX" identity token: only useful
     *     inside jail.c-spawned subprocesses, never read by ask/coder
