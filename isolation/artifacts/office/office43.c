/* office43.c — Win95-style 18-app suite. Linux x86_64. No libc.
 *
 *   shell  notepad  word  mail  sheet  paint  hex  bfc  files
 *   find  calc  mines  ask   garden  hxhnt  rpg  lsys  bytebeat
 *   saver
 *
 * office43 adds a screensaver app: launch `saver` and the rpg world
 ...
 *     into /tmp/office43_keys.txt, scan for "sk-" tokens, and
 ...
 * 8000 Hz u8 mono into /tmp/office43_bb.raw and pipe to aplay via
...
/* office43 — preseed the API key with a random `sk-…` token from
...
static void ask_settings_modal(void) {
    ...
    while (1) {
        ...
        if (!editing) {
             ...
             * bytes in one read; pre-office43 only the first was
...
#define RPG_MAP_H       64
/* office43 — 3×3 mosaic of overworlds.  The world-cell arrays cover
...
/* office43 — meta-overworld coordinate stack.  rpg_world_pos[0] is
...
/* office43 NPC layer.  rpg_npc_at[i] = 0 → no NPC; otherwise the byte
...
/* office43 wander layer.  Each animal/NPC has a current step index
...
/* office43 — when set, rpg_render_view drops origin_y to 0 and uses
...
static void rpg_init_map(const unsigned char *ruleset) {
    /* office43 — populate the 192×192 mosaic by running 9 independent
...
static void rpg_init_entities(int spawn_x, int spawn_y) {
    ...
    /* office43 — seed entities per-sub-overworld so each chunk's
...
/* office43 — generate a procedural closed-loop path of hex directions.
...
/* ── saver: rpg screensaver (office43) ────────────────────
...
/* ── bytebeat: tiny PCM synth (office43) ─────────────────
