/* office21.c — Win95-style 16-app suite. Linux x86_64. No libc.
 *
 *   shell  notepad  word  mail  sheet  paint  hex  bfc  files
 *   find  calc  mines  ask   garden  hxhnt  rpg  lsys
 *
 * New 16th app: lsys — a character-mode L-system viewer.  Six hard-
...
/* Render the visible 8×8 window centred on (px, py).  origin_y
 * leaves room for a one-row title; if the terminal isn't tall
 * enough for the full 8 rows of cells, we silently render fewer
 * (clipped at screen_h - 1). */
static void rpg_render_view(int px, int py, const unsigned char *pal) {
    ...
    /* Centre vertically too if the visible block is shorter than
     * the window we want — keeps the player roughly mid-screen. */
    ...
    for (int vy = 0; vy < rows_v; vy++) {
        ...
        for (int vx = 0; vx < RPG_VIS_W; vx++) {
            ...
            /* Three lines: top (no shift), middle (+RPG_MID_SHIFT),
             * bottom (no shift). */
            for (int line = 0; line < RPG_CELL_H; line++) {
                int line_x = base_x + (line == 1 ? RPG_MID_SHIFT : 0);
                cup(line_x, base_y + line);
                if (in_map) sgrbg(colour);
                else        sgrbg(0);     /* void = black */
                blanks(RPG_CELL_W);
            }
        }
    }

    /* Player glyph '@' on top of the centre cell.  Drawn last so
     * nothing overwrites it.  Use the genome accent colour for fg
     * so it stays visible under most palettes. */
    ...
    int wx_centre = px, wy_centre = py;
    int colour_under = 0;
    if (wx_centre >= 0 && wx_centre < RPG_MAP_W &&
        wy_centre >= 0 && wy_centre < RPG_MAP_H)
        colour_under = pal[rpg_map[wy_centre * RPG_MAP_W + wx_centre]];
    /* Middle line is shifted, so the player's column needs the same
     * +RPG_MID_SHIFT to land on the middle of the cell visually. */
...
/* Hex-aware player move: row-parity-sensitive deltas, identical to
 * garden's 'h' mode but on the 64×64 world. */
static void rpg_move(int *px, int *py, char c) {
    ...
    if (nx < 0 || nx >= RPG_MAP_W) return;
    if (ny < 0 || ny >= RPG_MAP_H) return;
    *px = nx; *py = ny;
}
...
static int run_rpg(int argc, char **argv) {
    ...
    while (1) {
        ...
        char hint[64]; int hp = 0;
        hp = sapp(hint, hp, " @ (");
        hp += utoa((unsigned)px, hint + hp); hint[hp++] = ',';
        hint[hp++] = ' ';
        hp += utoa((unsigned)py, hint + hp);
        hp = sapp(hint, hp, ")  w/e/a/d/z/c move · q quits ");
        hint[hp] = 0;
        status(hint);
        fbflush();
