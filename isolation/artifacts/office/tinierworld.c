/* tinierworld.c -- tinyworld trimmed: drop V-key hxhnt drop-in and
 * the lsys viewer alias.  Forked 2026-05-11 from tinyworld 84 KB to
 * recover ~10 KB by letting --gc-sections strip hx_display_seed,
 * hx_run_ga, hx_run_continuous_hunt, and the lsys: viewer at
 * line 8778 region (which only had the "lsys" dispatch alias
 * keeping it alive).  rpg/xpg/hxhnt aliases still all route to
 * run_xpg; what's gone is the V key inside run_xpg that used to drop
 * the player into hxhnt's full GA + scanning UI, and the standalone
 * `tinyworld lsys` viewer.  Sprite L-system bits the RPG uses
 * (rpg_sprites_init etc.) stay because rpg_render_view still calls
 * them — only the standalone lsys viewer goes.
 *
 * Original tinyworld banner follows.
 *
 * tinyworld.c -- tinyagentgrow + rpg/xpg/hxhnt/lsys entry points.
 *
 * Forked from tinyagentgrow 2026-05-11 (54.3 KB, Phase-5 + g-hotkey).
 * The rpg/hxhnt/garden/L-system helper bodies (rpg_render_view, hx_*,
 * lsys_*) already survived in tinyagentgrow's translation unit; what
 * got stripped along with the rest of the multi-app shell when
 * tinyagent was first minimised was the top-level run_xpg glue and
 * the argv[1] dispatch.  This fork ports them back so the same
 * binary that grows capability tails can also drop the user into the
 * tile explorer, the L-system viewer, or the hex-CA hunter.
 *
 * Everything else is verbatim tinyagentgrow:
 *  - tinyagentjail-style recursive sub-agent spawning + decompose-
 *    or-answer + leaf compile-verify + parent synthesis pipeline
 *  - "grow" subcommand family that extends the running binary with
 *    new capability tails using the same magic-bytes pattern used
 *    elsewhere in the office suite to embed CA genomes.
 *
 * Idea: each tail is a self-describing record at the end of the
 * binary; multiple tails chain back-to-back; on disk the binary
 * carries every capability it has earned via verified builds.  Two
 * tinyagentgrow instances forked from the same baseline and given
 * different tasks accumulate different tails and develop divergent
 * competence profiles — individuality through accreted skill, not
 * just runtime state.
 *
 * Tail format (each tail self-described in its trailer at EOF):
 *
 *      ┌─────────────────────────┐
 *      │ payload bytes (raw)     │  payload_len bytes
 *      ├─────────────────────────┤
 *      │ name[32], null-padded   │  32 bytes
 *      │ payload_len (u64 LE)    │   8 bytes
 *      │ magic "TAGW0001"        │   8 bytes
 *      └─────────────────────────┘
 *
 * Scanning: read last 48 bytes; if magic matches, record (name,
 * len), step back 48 + len, repeat.  Stop when magic doesn't match
 * (you've hit the original ELF or an older binary without tails).
 *
 * Phase-1 scaffold (this commit) — plumbing + self-test only:
 *   grow list         scan self, print tails (one line each)
 *   grow self-test    append a small text tail to self, write to
 *                     the next-numbered generation file, scan the
 *                     new file, confirm round-trip.  Proves the
 *                     read-self → append-tail → write-copy chain
 *                     works end-to-end with no LLM call.
 *
 * Phase 2 will wire the verifier chain (tinyagentjail Phase 3/4) to
 * auto-promote successful compiled binaries into tails; Phase 3 will
 * add `grow exec <name>` that extracts a tail to /tmp and invokes
 * it under jail.c sandboxing.  Phase 4 (eventual): in-process
 * mmap+JIT instead of extract-and-exec.
 *
 ...
 * picks when to swap in the upgraded generation.  Live-exec via an
 * additional `execve(new_path, …)` step is a one-line extension when
 * we want it.
 *
 * Same coder + ask + jail feature set as tinyagentjail.  Linux
 * x86_64, no libc.
 *
 * Trim chronology:
 *
 ...
 *   tinyagent  (+ packed linker script):31 448 B  (30.7 KB)
 *   tinyagentjail (Phase-4 baseline):   40 792 B  (39.8 KB)
 *   tinyagentgrow (this fork):          TBD       (target <64 KB)
 *
 * Build (uses tinyagentgrow.ld to pack segments):
 ...
 *      -Wl,-T,tinyagentgrow.ld -s
 *      -o tinyagentgrow tinyagentgrow.c
 *
 * Smoke tests:
 *   ./tinyagentgrow grow list        → "no tails" on a fresh build
 *   ./tinyagentgrow grow self-test   → appends a sample tail, writes
 *                                       tinyagentgrow.001, scans it,
 *                                       confirms round-trip.
 *
 ...
 * --- BEGIN ORIGINAL ---
 *
 * officeagent.c -- toy "Claude Code" classroom build.  Five visible
 * apps: shell / sheet / xpg / ask / prompt / coder.  Notepad, hex,
 * and files are still in the source (supercell carry-over) but
 * dropped from menus + dispatch; --gc-sections strips the
 * unreachable bodies.  Standalone hxhnt and lsys are dropped too —
 * the rpg → xpg merge from officex is applied here, so all CA + GA
 * + L-system logic is reachable only through xpg.
 *
...
#define pipe_(pfd)         sys3(SYS_pipe, (long)(pfd), 0, 0)
...
/* tinyworld: xpg subsumes the rpg/hxhnt/lsys aliases.  Body is
 * ported verbatim from tinyagentrpg.c — all called helpers
 * (hx_active_init, rpg_*, lsys_*, hx_display_seed, hx_run_*) are
 * already linked into the translation unit. */
...
/* ── tinyagentjail: recursive sub-agent spawning ────────
 *
 * A tinyagentjail process holds three budgets that bound how much
 * recursion it can do.  All three are tunable via CLI args and
 * decrement (or are inherited) on every spawn:
 *
 *   depth   = how many more levels of sub-spawn are allowed.
 *             Leaf at depth 0 must answer directly, no spawning.
 *   budget  = LLM token budget for this subtree.  Children get a
 *             share, return their consumption in their exit code or
 *             via the result string (Phase 2 wires this in).
 *   fanout  = max children to spawn at this level.  Default is RAM-
 *             sensed via /proc/meminfo MemAvailable so a small VM
 *             doesn't fork-bomb itself; capped at 8 even on big boxes.
 *
 * Phase-1 scaffold here gives us only fork+pipe+execve self-spawn
 * and a headless --jail-self-test mode that proves the pipeline.
 * Phase 2 will plug coder/ask's LLM call into a decompose-or-answer
 * prompt and dispatch through ja_spawn_self_collect().
 */
...
static int ja_atoi(const char *s) {
    int v = 0;
    while (*s >= '0' && *s <= '9') { v = v * 10 + (*s - '0'); s++; }
    return v;
}

static int ja_strpfx(const char *s, const char *pfx) {
...
/* Parse /proc/meminfo for MemAvailable: in KB.  Returns 0 on any
 * failure (caller falls back to a conservative default). */
static long ja_free_ram_kb(void) {
    int fd = (int)op("/proc/meminfo", O_RDONLY, 0);
    if (fd < 0) return 0;
    char buf[1024];
    long n = rd(fd, buf, (long)sizeof(buf) - 1);
    cl(fd);
    if (n <= 0) return 0;
    buf[n] = 0;
...
/* Default fan-out from free RAM.  Assume each tinyagentjail child
 * costs roughly 50 MB during its LLM round-trip (curl + response
 * buffer + bank + db).  Floor at 1 (always allow some progress),
 * cap at 8 (rate-limit + telephone-drift become real beyond that
 * even on a big box). */
static int ja_default_fan_out(void) {
    long kb = ja_free_ram_kb();
    if (kb <= 0) return 2;
    long n = kb / (50L * 1024L);
    if (n < 1) n = 1;
    if (n > 8) n = 8;
...
/* Splice --jail-* flags out of argv in-place.  Returns new argc.
 * Defaults: depth=3, budget=16000 tokens, fanout=RAM-sensed.  The
 * splice means downstream argv handling (ask/coder) sees the args
 * it always saw, with no jail flags polluting them. */
static int ja_parse(int argc, char **argv) {
    ja_cfg.depth     = 3;
    ja_cfg.budget    = 16000;
    ja_cfg.fanout    = ja_default_fan_out();
    ja_cfg.self_test = 0;
    int w = 0;
    for (int r = 0; r < argc; r++) {
        char *a = argv[r];
        if (!a) { argv[w++] = a; continue; }
        if (ja_strpfx(a, "--jail-depth=")) {
            ja_cfg.depth = ja_atoi(a + 13);
            continue;
        }
        if (ja_strpfx(a, "--jail-budget=")) {
            ja_cfg.budget = ja_atoi(a + 14);
            continue;
        }
        if (ja_strpfx(a, "--jail-fanout=")) {
            ja_cfg.fanout = ja_atoi(a + 14);
            continue;
        }
        if (scmp(a, "--jail-self-test") == 0) {
            ja_cfg.self_test = 1;
            continue;
        }
        argv[w++] = a;
...
/* Resolve /proc/self/exe → out (null-terminated).  Returns length or
 * -1.  Used to execve self for sub-agent spawning. */
static long ja_self_path(char *out, long cap) {
    long n = sys3(SYS_readlink, (long)"/proc/self/exe", (long)out, cap - 1);
    if (n < 0) return -1;
    out[n] = 0;
    return n;
}

/* Spawn self with xargv, capture stdout into out (null-terminated).
 * Returns child exit status (0 on clean exit), -1 on plumbing failure.
 * total_out (if non-NULL) gets the byte count read. */
static int ja_spawn_self_collect(char *const xargv[],
                                 char *out, long out_cap,
                                 long *total_out) {
    int pfd[2];
    if (pipe_(pfd) < 0) return -1;
    long pid = forkk();
    if (pid < 0) { cl(pfd[0]); cl(pfd[1]); return -1; }
    if (pid == 0) {
        cl(pfd[0]);
        dup2_(pfd[1], 1);
        cl(pfd[1]);
        execvee(xargv[0], xargv, g_envp);
        qu(127);
    }
    cl(pfd[1]);
    long total = 0;
    while (total < out_cap - 1) {
        long n = rd(pfd[0], out + total, out_cap - 1 - total);
        if (n <= 0) break;
        total += n;
    }
    out[total] = 0;
...
/* Phase-1 self-test.  At depth 0 (leaf), print "leaf d=0 task=<task>"
 * and exit.  At depth > 0 (parent), fan out N=fanout children with
 * depth-1, read their stdout, indent + echo upward.  Proves the
 * fork+pipe+execve recursion works end-to-end with no LLM in the
 * loop. */
static int ja_run_self_test(int argc, char **argv) {
    char d_s[8]; ja_utoa((unsigned int)ja_cfg.depth, d_s);

    if (ja_cfg.depth <= 0) {
        const char *task = argc > 1 && argv[1] ? argv[1] : "(none)";
        wr(1, "leaf d=0 task=", 14);
        wr(1, task, slen(task));
        wr(1, "\n", 1);
        return 0;
    }
    ...
    static char child_out[32768];
    for (int i = 0; i < ja_cfg.fanout; i++) {
        char task_s[16] = "subtask-X";
        task_s[8] = '0' + (char)i;
        task_s[9] = 0;
        char *xargv[8] = {
            self_path, (char *)"--jail-self-test",
            depth_arg, fanout_arg, task_s, 0, 0, 0
        };
        long got = 0;
        int rc = ja_spawn_self_collect(xargv, child_out,
                                       (long)sizeof(child_out), &got);
        if (rc < 0) {
            wr(2, "ja: spawn failed\n", 17);
            continue;
        }
        /* Indent child output one space per level the child was at. */
        long start = 0;
        for (long j = 0; j <= got; j++) {
            if (j == got || child_out[j] == '\n') {
                if (j > start) {
                    wr(1, "  ", 2);
                    wr(1, child_out + start, j - start);
                }
                if (j < got) wr(1, "\n", 1);
                start = j + 1;
            }
...
/* ── tinyagentgrow: capability-tail accumulation ────────
 *
 * Each tail is self-described in its trailer at end-of-file (see
 * the top of this file for the diagram).  Scanning walks backward
 * from EOF using the trailer's payload_len field; stopping when the
 * magic at the next trailer doesn't match means we've hit the
 * original ELF or an older tail-less binary.  Multiple tails chain
 * back-to-back with no global footer or table — each one is fully
 * self-contained.
 *
 * Phase 1 = plumbing only: list + self-test that proves the
...
static void tg_pack_u64_le(char *out, unsigned long v) {
    for (int i = 0; i < 8; i++) { out[i] = (char)(v & 0xff); v >>= 8; }
}

static unsigned long tg_unpack_u64_le(const char *in) {
    unsigned long v = 0;
    for (int i = 7; i >= 0; i--)
        v = (v << 8) | (unsigned char)in[i];
    return v;
}
...
/* Load a whole file into tg_buf.  Returns size on success or -1.
 * Caps at TG_BUF_CAP to keep the buffer manageable. */
static long tg_load_file(const char *path) {
    int fd = (int)op(path, O_RDONLY, 0);
    if (fd < 0) return -1;
    long size = sys3(SYS_lseek, fd, 0, 2);   /* SEEK_END */
    sys3(SYS_lseek, fd, 0, 0);                /* SEEK_SET */
    if (size <= 0 || size > (long)sizeof tg_buf) { cl(fd); return -1; }
    long got = 0;
    while (got < size) {
        long n = rd(fd, tg_buf + got, size - got);
        if (n <= 0) break;
        got += n;
    }
    cl(fd);
...
static int tg_subcmd_list(void) {
    ...
    if (ja_self_path(self_path, sizeof self_path) < 0) {
        wr(2, "tg: readlink /proc/self/exe failed\n", 35);
        return 1;
    }
    long size = tg_load_file(self_path);
    if (size < 0) {
        wr(2, "tg: failed to load self\n", 24);
        return 1;
    }
...
static int tg_append_to_copy(const char *name, char type,
    ...
    long w = 0;
    while (w < self_size) {
        long n = wr(fd, tg_buf + w, self_size - w);
 ...
 * next-generation path, reopen the new file, scan its tail chain,
 * confirm the new tail is present and readable.  Proves the whole
 * pipeline end-to-end with no LLM involved. */
static int tg_subcmd_self_test(void) {
    static const char payload[] =
        "tinyagentgrow self-test capability payload\n"
        "If you are reading this from the .NNN file's tail, the\n"
        "read-self -> append -> write-copy -> rescan loop is sound.\n";
    ...
    wr(1, "appended 'growtest_hello' (", 27);
    ...
    long size = tg_load_file(new_path);
    if (size < 0) {
        wr(2, "self-test: cannot reopen new file\n", 34);
        return 1;
    }
    int count = tg_count_tails(tg_buf, size);
    wr(1, "new-gen tail count: ", 20);
    char c_s[8]; ja_utoa((unsigned int)count, c_s);
    wr(1, c_s, slen(c_s));
    wr(1, "\n", 1);

    if (count < 1) {
        wr(2, "self-test: tail not found in new-gen file\n", 42);
        return 1;
    }
    ...
    for (long i = 0; i < plen; i++) {
        if (got_payload[i] != payload[i]) {
            wr(2, "self-test: byte mismatch\n", 25);
            return 1;
        }
    }
    wr(1, "self-test OK — round-trip verified\n", 37);
    return 0;
}
...
/* ── xpg: rpg + hxhnt + lsys entry point ──────────────────
 * Ported verbatim from tinyagentrpg.c run_xpg.  All called helpers
 * (hx_active_init, rpg_sprites_init, rpg_load_overworld,
 * rpg_player_init, rpg_preload_invalidate, rpg_terrain_anim_init,
 * rpg_animate_step, rpg_render_view, rpg_show_inventory,
 * rpg_cast_zap, rpg_anim_reset, hx_display_seed, hx_run_ga,
 * hx_run_continuous_hunt, rpg_palettes_refresh,
 * rpg_show_anim_settings, rpg_bend, rpg_move, rpg_path_tick,
 * rpg_preload_advance_one, rpg_shift_mosaic) are already defined
 * earlier in this file as carry-overs from the officeagent ancestor;
 * tinyagentgrow lacked only this glue and the dispatch line. */
static int run_xpg(int argc, char **argv) {
    ...
    while (1) {
        ...
        /* tinierworld: V-key drop-into-hxhnt removed.  Frees ~10 KB
         * via --gc-sections by letting hx_display_seed + hx_run_ga +
         * hx_run_continuous_hunt fall out of the call graph.  The
         * hxhnt GA/scanner UI is no longer reachable from this
         * binary; use ./tinyworld (the un-trimmed sibling) if you
         * need it. */
        if (k[0] == 'k' || k[0] == 'K') {
...
int main_c(int argc, char **argv, char **envp) {
    g_envp = envp;
    /* Splice --jail-* flags out of argv before any term init — the
     * self-test path is headless (no termios) and the recursive
     * spawn primitive must work whether or not stdin is a tty. */
    argc = ja_parse(argc, argv);
    if (ja_cfg.self_test) {
     ...
     * subsumes rpg + hxhnt + lsys via run_xpg. */
    if (argc >= 2 && argv[1]) {
        if (scmp(argv[1], "ask")   == 0) return run_ask  (argc - 1, argv + 1);
        if (scmp(argv[1], "coder") == 0) return run_coder(argc - 1, argv + 1);
        /* tinierworld: dropped "lsys" alias so the standalone L-system
         * viewer at line 8778 region gets --gc-sections'd.  RPG's
         * L-system sprites stay alive via rpg_sprites_init's call
         * graph, so the in-game rendering is unaffected. */
        if (scmp(argv[1], "xpg")   == 0 || scmp(argv[1], "rpg") == 0 ||
            scmp(argv[1], "hxhnt") == 0)
            return run_xpg(argc - 1, argv + 1);
    }
