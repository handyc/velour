/* office37.c — Win95-style 16-app suite. Linux x86_64. No libc.
 *
 *   shell  notepad  word  mail  sheet  paint  hex  bfc  files
 *   find  calc  mines  ask   garden  hxhnt  rpg  lsys
 *
 * office37 turns rpg's overworld into one cell of a larger overworld.
 * The 64×64 map you walk in is the level-1 view; that map is itself
 * a single cell of a 64×64 *meta-overworld*, which is in turn a cell
 * of a meta-meta-overworld, and so on up to 64 levels.  Every map at
 * every level is procedurally seeded from its absolute position
 * vector, so the world is deterministic but practically infinite.
 *
 * Walking off the east edge of the current overworld doesn't block
 * any more — it advances the level-1 x-coordinate by 1 (wrapping at
 * 64 with carry into level 2, etc.) and re-seeds the map.  The
 * player lands on the corresponding edge of the new tile.  Crossing
 * back returns the same content (same coords → same hash → same
 * map).  Levels above 1 don't have a separate "view" yet; they're
 * the addressing system that gives every overworld a unique seed.
 * A future fork can render the meta level by zooming out, run a GA
 * over level-2 cells, etc.
 *
...
/* office37 — meta-overworld coordinate stack.  rpg_world_pos[0] is
...
static unsigned char rpg_cat_at[RPG_MAP_W * RPG_MAP_H];
static unsigned char rpg_idx_at[RPG_MAP_W * RPG_MAP_H];
...
static void rpg_init_map(const unsigned char *ruleset) {
    /* Random initial grid.  office37: re-seed from the world coord
     * stack on every call so each overworld is reproducible.  Earlier
     * forks fell back to rdtsc only when hx_rng_state was zero; we
     * still keep that as the very first ever-seed before world coords
     * exist, but rpg_load_overworld will overwrite it before calling. */
    if (!hx_rng_state) {
        unsigned long h, l;
        __asm__ volatile ("rdtsc" : "=d"(h), "=a"(l));
        hx_rng_state = ((unsigned long long)h << 32) | l | 1ULL;
...
static void rpg_init_entities(int spawn_x, int spawn_y) {
    ...
    for (int i = 0; i < RPG_MAP_W * RPG_MAP_H; i++) {
        ...
        rpg_cat_at[i] = (unsigned char)cat;
        if (cat) {
            int idx = (int)(hx_rand() & (RPG_CAT_VARIANTS - 1));
            rpg_idx_at[i] = (unsigned char)idx;
            if (rpg_cats[cat].cat == 'A') rpg_hp_at[i] = rpg_cats[cat].hp;
        }
    }
...
/* Re-seed the RNG from the world coord stack and regenerate map +
 * entities for the current overworld.  spawn (x, y) determines the
 * 3×3 clear-zone around the player's entry point. */
static void rpg_load_overworld(int spawn_x, int spawn_y) {
    hx_rng_state = rpg_world_seed();
...
static int rpg_move(int *px, int *py, char c, char *msg) {
    ...
    if (nx < 0 || nx >= RPG_MAP_W || ny < 0 || ny >= RPG_MAP_H) {
        int wdx = 0, wdy = 0;
        if (nx < 0)              { wdx = -1; nx += RPG_MAP_W; }
        else if (nx >= RPG_MAP_W){ wdx =  1; nx -= RPG_MAP_W; }
        if (ny < 0)              { wdy = -1; ny += RPG_MAP_H; }
        else if (ny >= RPG_MAP_H){ wdy =  1; ny -= RPG_MAP_H; }
        rpg_world_advance(wdx, wdy);
        rpg_load_overworld(nx, ny);
        *px = nx; *py = ny;
        msg[sapp(msg, 0, "crossed")] = 0;
        return 1;
    }
