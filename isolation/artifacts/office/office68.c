/* office68.c -- TUI dashboard for Velour-locals. Linux x86_64. No libc.
 *
 *   shell  notepad  sheet  hex  files  calc
 *   ask   coder  soul   garden  hxhnt  rpg  lsys  saver
 *
 * office68 — `coder` app: iterative LLM-driven C generator that
 * reuses ask's curl machinery, same blend officeagent.c shipped but
 * inside the broader office67 environment (full app menu + local
 * MoE soul).  No bank/tdb yet; v1 is just the iterate loop.
 *
 *   1. Read a goal.
 *   2. Build prompt: target + goal + (optional) previous draft +
 *      (optional) previous cc stderr.
 *   3. ask_call_curl, extract first ```c…``` block.
 *   4. Write /tmp/office_coder_attempt.c, fork+execve cc, capture err.
 *   5. If compile fails, fold stderr into next prompt; cap at 4 iters.
 *
 * Targets: good_enough (cc compiles), clean (-Wall -Wextra clean),
 * perfect (clean + runs without crash).  Cycle with `t`.
 *
 * Deferred to office69+: 4-bank persistent memory (personality /
 * recent / longterm / project), TinyDB top-K retrieval, free-tier
 * key rotation, soul fallback on curl exhaustion.
 *
 * Carried forward from office67 — `soul` app: local mixture-of-experts via officemoe.
 *
 * Up until office66, the only LLM the suite could talk to was
 * `ask` — fork+execve curl to a remote OpenAI-compatible endpoint.
 * Useful but it requires API keys and network.  This fork adds a
 * sibling app `soul` that pipes the prompt to a local
 * /tmp/officemoe (or sibling-of-binary path) — the four-specialist
 * MoE + router-soul we shipped 2026-05-10.  No keys, no network,
 * no token cost; just a tiny trained transformer answering offline.
 *
 * Lookup order for the moe binary (first hit wins):
 *   1. Same dir as office67's own /proc/self/exe
 *   2. /tmp/officemoe
 *   3. ./officemoe (cwd)
 * Status line shows which path was found.
 *
 * UI is the same dual-pane chat as `ask`: scrollback above, prompt
 * line below.  Enter sends; output streams back into the chat as
 * "you: …" + "soul: …" exchanges.  ^N clears, ^Q exits.
 *
 * Carried forward from office66/65/.../59 — 64-bit math in calc + sheet evaluator.
 *
...
#ifndef OFFICE_FEATURE_NOTEPAD
#define OFFICE_FEATURE_NOTEPAD     1
#endif
#ifndef OFFICE_FEATURE_SHEET
#define OFFICE_FEATURE_SHEET       1
#endif
#ifndef OFFICE_FEATURE_HEX
#define OFFICE_FEATURE_HEX         1
#endif
#ifndef OFFICE_FEATURE_FILES
#define OFFICE_FEATURE_FILES       1
#endif
#ifndef OFFICE_FEATURE_CALC
#define OFFICE_FEATURE_CALC        1
#endif
#ifndef OFFICE_FEATURE_ASK
#define OFFICE_FEATURE_ASK         1
#endif
#ifndef OFFICE_FEATURE_SOUL
#define OFFICE_FEATURE_SOUL        1
#endif
#ifndef OFFICE_FEATURE_CODER
#define OFFICE_FEATURE_CODER       1
#endif
#ifndef OFFICE_FEATURE_GARDEN
#define OFFICE_FEATURE_GARDEN      1
#endif
#ifndef OFFICE_FEATURE_HXHNT
#define OFFICE_FEATURE_HXHNT       1
#endif
#ifndef OFFICE_FEATURE_RPG
#define OFFICE_FEATURE_RPG         1
#endif
#ifndef OFFICE_FEATURE_LSYS
#define OFFICE_FEATURE_LSYS        1
#endif
#ifndef OFFICE_FEATURE_SCREENSAVER
#define OFFICE_FEATURE_SCREENSAVER 1
#endif
#ifndef OFFICE_FEATURE_NET
#define OFFICE_FEATURE_NET         1
#endif
#ifndef OFFICE_FEATURE_HTTP
#define OFFICE_FEATURE_HTTP        1
#endif
#ifndef OFFICE_FEATURE_ECHO
#define OFFICE_FEATURE_ECHO        1
#endif
#ifndef OFFICE_FEATURE_FINGER
#define OFFICE_FEATURE_FINGER      1
#endif
#ifndef OFFICE_FEATURE_GOPHER
#define OFFICE_FEATURE_GOPHER      1
#endif
#ifndef OFFICE_FEATURE_PROBE
#define OFFICE_FEATURE_PROBE       1
#endif
#ifndef OFFICE_FEATURE_DNS
#define OFFICE_FEATURE_DNS         1
#endif
#ifndef OFFICE_FEATURE_FTP
#define OFFICE_FEATURE_FTP         1
#endif
#ifndef OFFICE_FEATURE_SSHTEL
#define OFFICE_FEATURE_SSHTEL      1
#endif
...
static const MS ms_ask     = { mF_ask, NA(mF_ask), 0, 0,
                               0, 0, mH_about, NA(mH_about) };
/* soul: New = clear chat, Quit. */
static const MI mF_soul[]  = {{"New     ^N", MA_NEW},
                              {"Quit    ^Q", MA_QUIT}};
static const MS ms_soul    = { mF_soul, NA(mF_soul), 0, 0,
                               0, 0, mH_about, NA(mH_about) };
...
static void show_about(const char *title) {
    ...
    body_at(2, 6, "  ask soul garden hxhnt rpg lsys saver", SCREEN_W - 4);
...
static int run_shell(int argc, char **argv) {
    ...
    while (running) {
        ...
        body_at(2, 4, "  notepad  sheet  hex  files  calc",
                SCREEN_W - 4);
        body_at(2, 5, "  ask  soul  garden  hxhnt  rpg  lsys",
                SCREEN_W - 4);
...
#endif
#if OFFICE_FEATURE_SOUL
            if (!matched && scmp(cmd, "soul") == 0)    { rc = run_soul(sub_argc, sub_argv); matched = 1; }
#endif
#if OFFICE_FEATURE_CODER
            if (!matched && scmp(cmd, "coder") == 0)   { rc = run_coder(sub_argc, sub_argv); matched = 1; }
#endif
...
/* office67: when set, ask_render_history uses "soul>" instead of
 * "ai>" for role-1 messages.  Lets soul share ask's chat buffer
 * + scroll logic without forking a parallel renderer. */
static int ask_role1_is_soul;
...
/* ── soul: local 4-specialist MoE via officemoe child ────
 *
 * Same dual-pane chat as `ask` (reuses ask_buf + ask_render_history)
 * but the assistant is /tmp/officemoe — the router-soul + 4
 * float-precision specialists shipped 2026-05-10.  No api key, no
 * network, no token cost; one fork+execve+pipe round-trip per turn.
 *
 * Lookup order for the moe binary (first hit wins):
 *   1. Same dir as office67's /proc/self/exe
 *   2. /tmp/officemoe
 *   3. ./officemoe (cwd)
 *
 * If none found, the chat shows a once-only error in red and the
 * user can quit.  No retry/backoff logic — re-launching `soul`
 * tries again from scratch. */

#define SOUL_INPUT_CAP   1024
#define SOUL_RESP_CAP    4096

static int soul_file_exists(const char *path) {
    int fd = (int)op(path, O_RDONLY, 0);
    if (fd < 0) return 0;
    cl(fd);
    return 1;
...
static int soul_resolve_moe(char *out, int cap) {
    /* 1) sibling of office67 */
    long n = sys3(SYS_readlink, (long)"/proc/self/exe",
                  (long)out, cap - 1);
    if (n > 0) {
        out[n] = 0;
        int slash = -1;
        for (int i = 0; i < n; i++) if (out[i] == '/') slash = i;
        if (slash >= 0) {
            out[slash + 1] = 0;
            int p = slash + 1;
            const char *name = "officemoe";
            int nl = slen(name);
            if (p + nl + 1 <= cap) {
                for (int i = 0; i < nl; i++) out[p + i] = name[i];
                out[p + nl] = 0;
    ...
    /* 2) /tmp/officemoe */
    static const char p2[] = "/tmp/officemoe";
    if (soul_file_exists(p2)) {
        int n2 = (int)sizeof p2 - 1;
        if (n2 + 1 <= cap) {
            for (int i = 0; i < n2; i++) out[i] = p2[i];
            out[n2] = 0;
            return 0;
        }
    }
...
static int soul_invoke_moe(const char *moe_path, const char *prompt,
    ...
    int got = 0;
    while (got < cap - 1) {
        long m = rd(pipefd[0], resp + got, cap - 1 - got);
        if (m <= 0) break;
        got += (int)m;
    }
    resp[got] = 0;
...
static int run_soul(int argc, char **argv) {
    ...
    int have_moe = (soul_resolve_moe(moe_path, sizeof moe_path) == 0);

    static char input[SOUL_INPUT_CAP];
    int inlen = 0;
    static char errmsg[128];
    errmsg[0] = 0;

    /* Reuse ask_buf for chat history; soul gets its own prefix. */
    ask_role1_is_soul = 1;
    ask_n_msgs = 0;
    ask_buf_use = 0;
    ...
    while (1) {
        ...
        if (errmsg[0]) {
            ...
            status("officemoe not found — see soul docstring for paths");
        } else {
            char sb[80];
            int sl_ = sapp(sb, 0, "ENTER send | ^N clear | ^Q quit | moe=");
            int pl = slen(moe_path);
            int show = pl > 30 ? 30 : pl;
            int from = pl - show;
            for (int i = 0; i < show && sl_ < (int)sizeof sb - 1; i++)
                sb[sl_++] = moe_path[from + i];
            sb[sl_] = 0;
            status(sb);
        }
        fbflush();
        ...
        int act = -1, ami = menu_activation(k, n);
        if (ami >= 0) act = menu_run(&ms_soul, ami);
        if (act == MA_ABOUT) { show_about("Soul"); continue; }
        if (act == MA_QUIT)  break;
        if (act == MA_NEW)   { ask_n_msgs = 0; ask_buf_use = 0; continue; }
        ...
        if (k[0] == '\r' || k[0] == '\n') {
            ...
            if (!have_moe) {
                int el = sapp(errmsg, 0,
                              "officemoe missing; build via build_officemoe.sh");
                errmsg[el] = 0;
                continue;
            }
            ask_msg_add(0, input, inlen);
            char saved[SOUL_INPUT_CAP];
            int saved_n = inlen;
            mcpy(saved, input, inlen);
            saved[inlen] = 0;
            inlen = 0;
            input[0] = 0;
            ...
            static char resp[SOUL_RESP_CAP];
            int rn = 0;
            int rc = soul_invoke_moe(moe_path, saved, resp, sizeof resp, &rn);
            if (rc < 0 || rn <= 0) {
                int el = sapp(errmsg, 0,
                              "officemoe call failed — check build / path");
                errmsg[el] = 0;
                ask_msg_add(1, "(no response)", 13);
            } else {
                /* Trim trailing whitespace/newlines. */
                while (rn > 0 && (resp[rn-1] == '\n' || resp[rn-1] == '\r'
                               || resp[rn-1] == ' ')) rn--;
                resp[rn] = 0;
                if (rn == 0) ask_msg_add(1, "(empty)", 7);
                else         ask_msg_add(1, resp, rn);
            }
            continue;
        }

        if (k[0] == 0x7f || k[0] == 0x08) { /* backspace */
            if (inlen > 0) { inlen--; input[inlen] = 0; }
            continue;
        }
        if (k[0] >= 32 && k[0] < 127 && inlen < SOUL_INPUT_CAP - 1) {
            input[inlen++] = (char)k[0];
            input[inlen] = 0;
        }
    }
...
#if OFFICE_FEATURE_CODER
/* ── coder: iterative LLM-driven C generator (v1, scaffold) ──
 * Full implementation lands in the next commit; this stub keeps
 * dispatch wiring + menu hooks live so the rest of the suite
 * compiles. */
static int run_coder(int argc, char **argv) {
    ...
    chrome("Coder");
    body_clear();
    body_at(2, 3, "coder app — scaffold (v1 in next commit).",
            SCREEN_W - 4);
    body_at(2, 5, "  reuses ask_call_curl; iterative compile loop;",
            SCREEN_W - 4);
    body_at(2, 6, "  goal -> ask -> extract code -> cc -> retry.",
            SCREEN_W - 4);
