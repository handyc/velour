/* office27.c — Win95-style 16-app suite. Linux x86_64. No libc.
 *
 *   shell  notepad  word  mail  sheet  paint  hex  bfc  files
 *   find  calc  mines  ask   garden  hxhnt  rpg  lsys
 *
 * office27 fixes the rpg SE-movement key.  office20 mapped SE to
...
static int run_rpg(int argc, char **argv) {
    ...
    while (1) {
        ...
        hl = sapp(hint, hl, "wadezx=move i=inv m=zap q=quit ");
        hint[hl] = 0;
        status(hint);
        fbflush();
