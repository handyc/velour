/* office20.c — Win95-style 15-app suite. Linux x86_64. No libc.
 *
 *   shell  notepad  word  mail  sheet  paint  hex  bfc  files
 *   find  calc  mines  ask   garden  hxhnt  rpg
 *
 * New 15th app: rpg — a tiny tile-based explorer that turns the
...
static int run_shell(int argc, char **argv) {
    ...
    while (running) {
        ...
        body_at(2, 4, "  notepad  word  mail  sheet  paint  hex  bfc",
                SCREEN_W - 4);
        body_at(2, 5, "  files  find  calc  mines  ask  garden  hxhnt  rpg  exit",
                SCREEN_W - 4);
