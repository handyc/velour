/* officecoder.c -- fork of officeagent stripped to ask + coder only.
 * Linux x86_64.  No libc.  Built with same -Os --gc-sections flags
 * as officeagent; the linker drops the bodies of every other app.
 *
 *   ask  coder      (default subcommand: coder)
 *
 * Forked from officeagent 2026-05-10.  All officeagent apps stay
 * in the source unmodified — diffing officecoder.c against
 * officeagent.c shows only the trimmed dispatch in main_c, plus
 * the deleted office..office50 legacy basename chain.  --gc-sections
 * does the rest: run_shell, run_sheet, run_xpg, run_garden,
 * run_preview_genome, run_view_genome, run_soul, run_prompt, and
 * all the supporting hxhnt / rpg / sheet / garden engine code drop
 * away because nothing in the call graph reaches them.
 *
 * Result: 37 KB binary with the FULL coder feature set:
 ...
 * plus full ask (multi-provider HTTPS, auto-key fetch from upstream
 * README, settings modal, conversation history scroll), 4-bank
 * persistent memory, TinyDB-backed top-K context retrieval.
 *
