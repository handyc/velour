/* tinyagentjail.c -- tinyagent + recursive sub-agent spawning.
 *
 * Forked from tinyagent 2026-05-10 (30.7 KB locked baseline).  Adds
 * a fork+pipe self-spawn primitive so a tinyagentjail process can
 * recursively dispatch sub-tasks to child tinyagentjails, each
 * running under a smaller depth/token budget than its parent.
 * Leaves return text strings; parents aggregate and return upward.
 *
 * Intended domain is software construction (decompose design goal →
 * modules → functions → leaves that compile), so the natural verifier
 * at every level is "does this compile / pass its test?" rather than
 * a learned reward model.
 *
 * Phase-1 scaffold (this commit):
 *   --jail-depth=N    depth budget; decremented on every spawn
 *                     (default 3, leaf at 0 must answer directly)
 *   --jail-budget=N   token budget passed down (default 16000)
 *   --jail-fanout=N   max children per parent (default = RAM-sensed
 *                     via /proc/meminfo MemAvailable, capped at 8)
 *   --jail-self-test  headless smoke test: recursively self-spawn
 *                     and prove the fork+pipe+execve pipeline works
 *                     without any LLM call
 *
 * Phase 2: ask/coder issues a decompose-or-answer prompt; SUBTASKS
 * dispatch via the primitives here, ANSWER returns directly.
 * Phase 3: compile/test verifier at every level — parent rejects
 * child output that doesn't compile, re-prompts or re-decomposes.
 *
 * Same coder + ask feature set as tinyagent.  64 KB ceiling (not
 * 32 KB) — the spawning machinery and decomposition templates need
 * the headroom.  Linux x86_64, no libc.
 *
 * Trim chronology:
 *
 ...
 *   tinyagentjail (this fork):          TBD       (target <64 KB)
 *
 * Build (uses tinyagentjail.ld to pack segments):
 ...
 *      -Wl,-T,tinyagentjail.ld -s
 *      -o tinyagentjail tinyagentjail.c
 *
 * Smoke test:
 *   ./tinyagentjail --jail-self-test --jail-depth=2 --jail-fanout=2
 * should print one parent line at depth 2, fan out 2 children at
 * depth 1 (each fans out 2 grandchildren), 4 leaves at depth 0
 * each printing their assigned subtask name.
 *
