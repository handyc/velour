/* office38.c — Win95-style 16-app suite. Linux x86_64. No libc.
 *
 *   shell  notepad  word  mail  sheet  paint  hex  bfc  files
 *   find  calc  mines  ask   garden  hxhnt  rpg  lsys
 *
 * office38 seeds NPCs into the rpg overworld.  An NPC is a 1×2
...
/* office38 — meta-overworld coordinate stack.  rpg_world_pos[0] is
...
/* office38 NPC layer.  rpg_npc_at[i] = 0 → no NPC; otherwise the byte
...
static void rpg_init_map(const unsigned char *ruleset) {
    /* Random initial grid.  office38: re-seed from the world coord
...
static void rpg_init_entities(int spawn_x, int spawn_y) {
    ...
    for (int i = 0; i < RPG_MAP_W * RPG_MAP_H; i++) {
        ...
        /* office38 — sprinkle NPCs onto empty soil/sand at ~3% rate.
