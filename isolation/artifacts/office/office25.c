/* office25.c — Win95-style 16-app suite. Linux x86_64. No libc.
 *
 *   shell  notepad  word  mail  sheet  paint  hex  bfc  files
 *   find  calc  mines  ask   garden  hxhnt  rpg  lsys
 *
 * office25 makes the nested CA actually visible.  office23/24 sampled
...
/* The .hxseed default genome is all zeros, which makes hx_g_get
 * return 0 for every situation — one CA step collapses any input
 * grid to uniform state 0.  Cache the live/empty flag so we only
 * scan once. */
...
static int rpg_genome_is_live(void) {
    if (rpg_genome_live_cache == -1) {
        ...
        for (int i = 0; i < HX_GBYTES; i++)
            if (hx_embedded_genome[i]) { rpg_genome_live_cache = 1; break; }
    }
