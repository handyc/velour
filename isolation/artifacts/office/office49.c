/* office49.c -- Win95-style 12-app suite. Linux x86_64. No libc.
 *
 *   shell  notepad  sheet  hex  files  calc
 *   ask   garden  hxhnt  rpg  lsys  bytebeat  saver
 *
 * office49 trims back under the 64 KB cap.  Six apps removed:
 *   - mail   never had real SMTP send, just a UI
 *   - find   single-pass substring grep, no regex
 *   - paint  ASCII canvas demo
 *   - word   duplicated notepad with a paragraph reflow
 *   - mines  16x16 minesweeper toy
 *   - bfc    brainfuck compiler -- fun but rarely launched
 * Net trim ~6900 B (the 5 textual apps got us to 66912 -- still 1376
 * over due to page alignment of .data; bfc tipped us across the next
 * 4 KB boundary).  Stripped binary lands at 62816 B with 2720 B of
 * headroom under the 65536 cap.
 *
 ...
 *     into /tmp/office49_keys.txt, scan for "sk-" tokens, and
 ...
 * 8000 Hz u8 mono into /tmp/office49_bb.raw and pipe to aplay via
...
static void show_about(const char *title) {
    ...
    body_at(2, 6, "  ask garden hxhnt rpg lsys bytebeat saver", SCREEN_W - 4);
...
/* office49 — provider-aware key grabber.  Pulls the constantly-
...
static void ask_fetch_random_key(void) {
    ...
    /* office49 — pull the model name from the same row.  The README
...
static void ask_settings_modal(void) {
    ...
    while (1) {
        ...
        if (!editing) {
             ...
             * bytes in one read; pre-office49 only the first was
...
#define RPG_MAP_H       64
/* office49 — 3×3 mosaic of overworlds.  The world-cell arrays cover
...
/* office49 — meta-overworld coordinate stack.  rpg_world_pos[0] is
...
/* office49 NPC layer.  rpg_npc_at[i] = 0 → no NPC; otherwise the byte
...
/* office49 wander layer.  Each animal/NPC has a current step index
...
/* office49 — when set, rpg_render_view drops origin_y to 0 and uses
...
static void rpg_init_map(const unsigned char *ruleset) {
    /* office49 — populate the 192×192 mosaic by running 9 independent
...
static void rpg_init_entities(int spawn_x, int spawn_y) {
    ...
    /* office49 — seed entities per-sub-overworld so each chunk's
...
/* office49 — generate a procedural closed-loop path of hex directions.
...
/* ── saver: rpg screensaver (office49) ────────────────────
...
/* ── bytebeat: tiny PCM synth (office49) ─────────────────
