/* office7.c — Win95-style 13-app suite. Linux x86_64. No libc.
 *
 *   shell  notepad  word  mail  sheet  paint  hex  bfc  files
 *   find  calc  mines  ask   garden
 *
 * Same apps + fixes as office6 plus two new apps (one per commit):
 *   - ask — dual-pane LLM chat. History above, single-line input
 *     below. Talks to OpenAI-compatible Chat Completions endpoints
 *     via execve("curl", ...) (no in-process TLS). Settings live
 *     in ./office7.conf (api_key=, endpoint=, model=) and are
 *     editable through File -> Settings inside the app.
 *   - garden — interactive-evolution colour/layout breeder.
 *     8x8 grid of 64 genome thumbnails; user marks favourites,
 *     ENTER breeds the next generation. Selected genome can be
 *     previewed full-screen (paints the suite chrome with that
 *     genome's colour palette + clock corner + menu order).
 *     Genome saved to ./garden.bin.
 */
