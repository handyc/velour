/* office12.c — Win95-style 13-app suite. Linux x86_64. No libc.
 *
 *   shell  notepad  word  mail  sheet  paint  hex  bfc  files
 *   find  calc  mines  ask   garden
 *
 * One-line change in spirit: replaces the hard-coded "office7" /
 * "office9" / etc. literals scattered through earlier forks with a
 * single APP_NAME #define.  Adjacent C string literals concatenate
 * at compile time, so ASK_CONF, ASK_REQ_FILE, ASK_RESP_FILE, and
 * the show_about line all resolve to the right per-fork strings
 * with no runtime cost.  Future forks just bump the #define.
 *
