/* office54.c -- Win95-style 11-app suite. Linux x86_64. No libc.
 *
 *   shell  notepad  sheet  hex  files  calc
 *   ask   garden  hxhnt  rpg  lsys  saver
 *
 * office54 — drop the bytebeat module to slim the suite back under
 * the 64 KB cap.  Bytebeat (office39) was a tiny 5-preset PCM synth
 * that rendered 4 s of u8 8000 Hz audio to /tmp and shelled out to
 * aplay.  Useful but optional, and the only suite app that touches
 * /tmp scratch + an external alsa binary; everything else is in-
 * process.  Removing it strips the bb_eval / bb_render_and_play /
 * run_bytebeat trio, the BB_* constants, the static 32 KB render
 * buffer (.bss only, no file impact), the forward decl, two menu
 * strings, and two dispatch entries.  Apps list goes 12 → 11.
 *
