/* office24.c — Win95-style 16-app suite. Linux x86_64. No libc.
 *
 *   shell  notepad  word  mail  sheet  paint  hex  bfc  files
 *   find  calc  mines  ask   garden  hxhnt  rpg  lsys
 *
 * office24 fixes the nested-CA render glitch from office23: when the
 ...
 *
 * Movement: w=NW, e=NE, a=W, d=E, z=SW, c=SE.  s is reserved.  q
...
static void show_about(const char *title) {
    ...
    body_at(2, 6, "  files find calc mines ask garden hxhnt rpg lsys", SCREEN_W - 4);
...
static void hx_display_seed(const unsigned char *genome,
                            const unsigned char *pal, unsigned int gseed) {
    ...
    /* Show a transient export-confirmation line for the next ~5
     * frames after the user hits 'x'. */
    int flash_ttl = 0;
    ...
    for (long tick = 0; ; tick++) {
        ...
        if (flash_ttl > 0) {
            ...
            p = sapp(hint, p, "  pal=[");
            p += utoa((unsigned)pal[0], hint + p); hint[p++] = ' ';
            p += utoa((unsigned)pal[1], hint + p); hint[p++] = ' ';
            p += utoa((unsigned)pal[2], hint + p); hint[p++] = ' ';
            p += utoa((unsigned)pal[3], hint + p);
            p = sapp(hint, p, "]  x exports · q quits ");
        }
        hint[p] = 0;
        status(hint);
        fbflush();
        ...
        unsigned char k[8];
        int n = read_key(k, sizeof k);   /* returns within ~200 ms */
        if (n > 0) {
            if (k[0] == 0x1b || k[0] == 'q' || k[0] == 'Q') break;
            if (k[0] == 'x' || k[0] == 'X') {
                /* Splice the currently-loaded genome into a copy of
                 * /proc/self/exe and write under a fresh hxh-* name. */
                char nm[HX_EXPORT_NAME_LEN + 1];
                hx_make_export_name(nm, xseq++);
                int rc = hx_splice_export(nm, pal, genome);
                int q = 0;
                if (rc == 0) {
                    q = sapp(flash_msg, q, " exported ");
                    q = sapp(flash_msg, q, nm);
                } else {
                    q = sapp(flash_msg, q, " export failed ");
                }
                flash_msg[q] = 0;
                flash_ttl = 5;
            }
...
static int run_hxhnt(int argc, char **argv) {
    ...
    /* Seed RNG from rdtsc (matches garden's pattern). */
    if (!hx_rng_state) {
        unsigned long h, l;
        __asm__ volatile ("rdtsc" : "=d"(h), "=a"(l));
    ...
    /* Bootstrap a seed if hxhnt.seed is missing (random palette +
     * identity genome — exactly the original hunter behaviour). */
    /* Bootstrap order:
     *   1. ./hxhnt.seed if present (explicit user save)
     *   2. embedded payload if it carries a real spliced genome
     *      (detected: not all zeros — a fresh build emits a zero
     *      genome which collapses the grid to colour 0 in one step)
     *   3. random fallback so the default view *moves* even on a
     *      brand-new binary
     * Any way the seed is sourced, hx_seed_pal/genome ends up
     * primed before display or GA mode runs. */
    if (hx_read_seed(HX_SEED_FILE, hx_seed_pal, hx_seed_genome) == 0) {
        /* user-saved */
    } else {
        /* Probe the embedded genome — if it's been spliced, use it. */
        int embedded_live = 0;
        for (int i = 0; i < HX_GBYTES; i++) {
    ...
    /* Parse args from argv[1] (the shell passes everything after the
     * command as a single string). */
    int pop = 0, gens = 0;
    unsigned rseed = 42;
    int mode = (argc > 1) ? hx_parse_args(argv[1], &pop, &gens, &rseed) : 0;

    /* Display mode: animate the seed on a fresh random grid until
     * the user hits ESC or q. */
    if (mode <= 0) {
        hx_display_seed(hx_seed_genome, hx_seed_pal,
                        (unsigned int)time_());
        term_cooked();
        return 0;
    }
    ...
    for (int i = 1; i < pop; i++) {
        hx_mutate(hx_pool[i], hx_seed_genome, 838860u);   /* 0.05 × 2^24 */
    ...
    for (int gen = 0; gen < gens; gen++) {
        ...
        /* Allow ESC/q to abort during long GA runs. */
        unsigned char k[8];
        int n = read_key(k, sizeof k);
        ...
        for (int i = pop / 2; i < pop; i++) {
            ...
            hx_mutate(hx_pool[i], tmp, 83886u);            /* 0.005 × 2^24 */
            hx_palette_inherit(hx_pals[i], hx_pals[pa], hx_pals[pb]);
        }
    }

    /* Final tournament + winner writing. */
    for (int i = 0; i < pop; i++)
        hx_fit[i] = hx_fitness(hx_pool[i], rseed);
    hx_sort_by_fitness(pop);

    /* Save the new best as the live seed so subsequent display-mode
     * runs animate the evolved winner.  Always — independent of
     * whether the user manually exports any of the winners. */
    hx_write_seed(HX_SEED_FILE, hx_pals[0], hx_pool[0]);

    /* Manual-export winners screen.  Sits in a key loop until the
     * user presses q/ESC; meanwhile 1/2/3 each splice that winner
     * into a fresh runnable binary.  Re-export is allowed (the
     * sequence-suffix in the filename keeps each call's output
     * uniquely named). */
    int seq = 1;
    char status_msg[80];
    status_msg[0] = 0;
    while (1) {
        ...
        body_clear();
        body_at(2, 3, "Top winners — press 1/2/3 to export, q to return:",
                SCREEN_W - 4);
        ...
        else               status(" 1/2/3 splice export · q quits ");
        fbflush();
...
static unsigned char rpg_map[RPG_MAP_W * RPG_MAP_H];
static unsigned char rpg_buf[RPG_MAP_W * RPG_MAP_H];

/* Per-cell entity layer.  rpg_kind_at[i] = 0 → empty, otherwise an
 * index into rpg_kinds[].  rpg_hp_at[i] is only meaningful for
 * animals; trees/buildings/items are static. */
static unsigned char rpg_kind_at[RPG_MAP_W * RPG_MAP_H];
static unsigned char rpg_hp_at  [RPG_MAP_W * RPG_MAP_H];
 ...
 * keep the result + a 4-colour palette in the cache.  120 KB BSS
 * total; zero-init so it doesn't bloat the binary file. */
static unsigned char rpg_inner_a[RPG_MAP_W * RPG_MAP_H];
static unsigned char rpg_inner_b[RPG_MAP_W * RPG_MAP_H];
static unsigned char rpg_cell_sample[RPG_MAP_W * RPG_MAP_H * 24]; /* 8×3 */
static unsigned char rpg_cell_pal   [RPG_MAP_W * RPG_MAP_H * 4];
static unsigned char rpg_cell_done  [RPG_MAP_W * RPG_MAP_H];

/* Fixed terrain-type → base RGB table.  CA states 0..3 mean:
 *   0 water, 1 grass, 2 dirt, 3 lava/special. */
struct RpgRGB { unsigned char r, g, b; };
static const struct RpgRGB rpg_terrain_rgb[4] = {
    { 30,  90, 200},
    { 60, 180,  80},
    {160, 110,  60},
    {220,  60,  40},
...
/* xterm-256 colour cube: indices 16..231 form a 6×6×6 RGB cube. */
...
static void rpg_compute_cell(int wx, int wy) {
    int idx = wy * RPG_MAP_W + wx;
    if (rpg_cell_done[idx]) return;
    unsigned long s = rpg_cell_hash(wx, wy);
    /* Random 4-colour seed grid via the cell's own LCG.  Independent
     * of the global hxhnt RNG so overworld + inner stay reproducible. */
    for (int i = 0; i < RPG_MAP_W * RPG_MAP_H; i++)
        rpg_inner_a[i] = (unsigned char)(rpg_lcg_next(&s) & 3);
    int n_steps = (int)((rpg_lcg_next(&s) >> 8) % 3) + 2;   /* 2..4 */
    for (int t = 0; t < n_steps; t++) {
        rpg_step_grid(hx_embedded_genome, rpg_inner_a, rpg_inner_b);
        mcpy(rpg_inner_a, rpg_inner_b, RPG_MAP_W * RPG_MAP_H);
    }
    /* Sample 8×3 spread evenly across the inner grid. */
    unsigned char *sample = &rpg_cell_sample[idx * 24];
    for (int r = 0; r < 3; r++) {
        int iy = (r * RPG_MAP_H + RPG_MAP_H / 2) / 3;
        if (iy >= RPG_MAP_H) iy = RPG_MAP_H - 1;
        for (int c = 0; c < 8; c++) {
            int ix = (c * RPG_MAP_W + RPG_MAP_W / 2) / 8;
            if (ix >= RPG_MAP_W) ix = RPG_MAP_W - 1;
            sample[r * 8 + c] = rpg_inner_a[iy * RPG_MAP_W + ix];
        }
    }
...
/* Each EntityKind is a small ASCII sprite drawn with its bottom row
 * on the cell's bottom line, extending upward.  Tall sprites (height
 * > 3) paint into the cells visually above them, achieving the
 * occlusion the user asked for: a 5-row tree in cell (wx, wy) covers
 * the upper 2 rows of (wx, wy-1) too.  Sprites are 3 cols wide; ' '
 * cells in the art are transparent (the previously painted terrain
 * shows through). */
struct EntityKind {
    char  cat;          /* 'P' plant, 'B' building, 'A' animal, 'I' item */
    unsigned char height;
    unsigned char fg;
    unsigned char hp;          /* base HP for animals, 0 otherwise */
    unsigned char blocking;    /* 1 if walking onto it is blocked outright */
    const char *name;
    /* art = height lines × 3 cols, no separators (top line first). */
    const char *art;
};

static const struct EntityKind rpg_kinds[] = {
    {0,   0,   0, 0, 0, "none",  ""},
    {'P', 5,  46, 0, 1, "tree",
     " ^ "
     "/|\\"
...
#define RPG_KIND_N ((int)(sizeof rpg_kinds / sizeof rpg_kinds[0]))

/* Player state.  inv[k] counts of item kind k (only items track here);
 * inv_count is the running total for status-bar display. */
struct RpgPlayer {
    int hp, max_hp;
    int mp, max_mp;
    unsigned char inv[RPG_KIND_N];
    int inv_count;
};
...
static void rpg_init_map(const unsigned char *ruleset) {
    /* Random initial grid — uses hxhnt's RNG (seeded once). */
    if (!hx_rng_state) {
        unsigned long h, l;
        __asm__ volatile ("rdtsc" : "=d"(h), "=a"(l));
        hx_rng_state = ((unsigned long long)h << 32) | l | 1ULL;
    ...
    for (int i = 0; i < RPG_MAP_W * RPG_MAP_H; i++)
        rpg_map[i] = (unsigned char)(hx_rand() & 3);
    /* Step a few times to let the ruleset shape it. */
    for (int t = 0; t < RPG_GEN_STEPS; t++) {
        rpg_step_grid(ruleset, rpg_map, rpg_buf);
        mcpy(rpg_map, rpg_buf, sizeof rpg_map);
    }
}

/* Pick an entity kind for a cell, biased by its terrain colour.  r is
 * 0..255 from the same RNG as rpg_init_map. */
static int rpg_kind_for(int colour, unsigned r) {
    switch (colour & 3) {
    case 0:                          /* "forest" */
        if (r <  18) return 1;       /* tree     */
        if (r <  40) return 2;       /* bush     */
        if (r <  50) return 5;       /* deer     */
        if (r <  55) return 8;       /* gem      */
        return 0;
    case 1:                          /* "meadow" */
        if (r <  10) return 2;       /* bush     */
        if (r <  20) return 6;       /* bird     */
        if (r <  24) return 7;       /* chest    */
        return 0;
    case 2:                          /* "settlement" */
        if (r <  15) return 3;       /* house    */
        if (r <  22) return 4;       /* tower    */
        if (r <  28) return 7;       /* chest    */
        return 0;
    case 3:                          /* "wilds"  */
        if (r <   8) return 1;       /* tree     */
        if (r <  18) return 5;       /* deer     */
        if (r <  24) return 6;       /* bird     */
        if (r <  28) return 8;       /* gem      */
        return 0;
    }
    return 0;
}
...
static void rpg_init_entities(int spawn_x, int spawn_y) {
    mset(rpg_kind_at, 0, sizeof rpg_kind_at);
    mset(rpg_hp_at,   0, sizeof rpg_hp_at);
    for (int i = 0; i < RPG_MAP_W * RPG_MAP_H; i++) {
        int c = rpg_map[i];
        unsigned r = hx_rand() & 0xff;
        int k = rpg_kind_for(c, r);
        rpg_kind_at[i] = (unsigned char)k;
        if (rpg_kinds[k].cat == 'A') rpg_hp_at[i] = rpg_kinds[k].hp;
    }
    /* Clear a 3×3 around spawn so the player isn't boxed in. */
    for (int dy = -1; dy <= 1; dy++) for (int dx = -1; dx <= 1; dx++) {
        int nx = spawn_x + dx, ny = spawn_y + dy;
        if (nx >= 0 && nx < RPG_MAP_W && ny >= 0 && ny < RPG_MAP_H)
            rpg_kind_at[ny * RPG_MAP_W + nx] = 0;
    }
}
...
static void rpg_render_view(int px, int py) {
    ...
    /* Pass 2: entity sprites, north→south so southern (closer)
    ...
    for (int vy = 0; vy < rows_v; vy++) {
        ...
        for (int vx = 0; vx < RPG_VIS_W; vx++) {
            int wx = px + vx - RPG_PLAYER_VX;
            int wy = py + vy - RPG_PLAYER_VY;
            if (wx < 0 || wx >= RPG_MAP_W || wy < 0 || wy >= RPG_MAP_H) continue;
            int idx = wy * RPG_MAP_W + wx;
            int kind = rpg_kind_at[idx];
            if (!kind) continue;
            const struct EntityKind *e = &rpg_kinds[kind];
            /* Behind-entity bg = the cell's centre-line texture colour
             * (sample[1*8 + 3] is the chunk right under the sprite). */
            unsigned char *sample = &rpg_cell_sample[idx * 24];
            unsigned char *cpal   = &rpg_cell_pal   [idx * 4];
            unsigned char terrain_col = cpal[sample[11] & 3];
            int base_x = origin_x + row_offset + vx * RPG_CELL_W;
            int base_y = origin_y + vy * RPG_CELL_H;
            int art_x = base_x + (RPG_CELL_W - 3) / 2;
            int h = e->height;
            for (int li = 0; li < h; li++) {
                ...
                for (int col = 0; col < 3; col++) {
                    char ch = e->art[art_row * 3 + col];
                    if (ch == ' ') continue;
                    int sx = art_x + col;
                    if (sx < 0 || sx >= SCREEN_W) continue;
                    cup(sx, sy);
                    sgrbgfg(terrain_col, e->fg);
                    fbw(&ch, 1);
                }
            }
        }
    ...
    /* Player glyph '@' on top of the centre cell. */
    int row_offset = (RPG_PLAYER_VY & 1) ? (RPG_CELL_W / 2) : 0;
    int player_screen_x =
        origin_x + row_offset + RPG_PLAYER_VX * RPG_CELL_W + RPG_CELL_W / 2;
    int player_screen_y = origin_y + RPG_PLAYER_VY * RPG_CELL_H + 1;
    int colour_under = 0;
    if (px >= 0 && px < RPG_MAP_W && py >= 0 && py < RPG_MAP_H) {
        int idx = py * RPG_MAP_W + px;
        rpg_compute_cell(px, py);
        unsigned char *sample = &rpg_cell_sample[idx * 24];
        unsigned char *cpal   = &rpg_cell_pal   [idx * 4];
        colour_under = cpal[sample[11] & 3];
    }
    cup(player_screen_x + RPG_MID_SHIFT, player_screen_y);
    sgrbgfg(colour_under, g_genome.accent);
    fbs("@");
    sgr0();
}
...
static int rpg_move(int *px, int *py, char c, char *msg) {
    ...
    case 'c': ny = y + 1; nx = x + (odd ? 1 : 0);      break;
    default:  return 0;
    }
    if (nx < 0 || nx >= RPG_MAP_W) return 0;
    if (ny < 0 || ny >= RPG_MAP_H) return 0;
    int idx = ny * RPG_MAP_W + nx;
    int kind = rpg_kind_at[idx];
    if (kind) {
        const struct EntityKind *e = &rpg_kinds[kind];
        if (e->cat == 'I') {
        ...
        if (e->cat == 'P' && e->height <= 2) {
            /* Walk through small bushes. */
        } else if (e->blocking) {
...
static int rpg_cast_zap(int px, int py, char *msg) {
    ...
    for (int dy = -3; dy <= 3; dy++) for (int dx = -3; dx <= 3; dx++) {
        ...
        if (nx < 0 || nx >= RPG_MAP_W) continue;
        if (ny < 0 || ny >= RPG_MAP_H) continue;
        int idx = ny * RPG_MAP_W + nx;
        if (!rpg_kind_at[idx]) continue;
        if (rpg_kinds[rpg_kind_at[idx]].cat != 'A') continue;
        int adx = dx < 0 ? -dx : dx;
        int ady = dy < 0 ? -dy : dy;
        int d = adx + ady;
    ...
    int n = sapp(msg, 0, "zap! ");
    n = sapp(msg, n, rpg_kinds[rpg_kind_at[best_idx]].name);
    n = sapp(msg, n, " takes 6");
    if (hp == 0) {
        rpg_kind_at[best_idx] = 0;
        n = sapp(msg, n, "; killed!");
    }
    msg[n] = 0;
    return 1;
...
static void rpg_show_inventory(void) {
    ...
    int line = 0;
    for (int k = 1; k < RPG_KIND_N; k++) {
        if (rpg_kinds[k].cat != 'I') continue;
        if (!rpg_player.inv[k]) continue;
        cup(bx + 2, by + 2 + line);
        char buf[32]; int n = 0;
        n = sapp(buf, n, rpg_kinds[k].name);
        n = sapp(buf, n, "  x");
...
static int run_rpg(int argc, char **argv) {
    (void)argc; (void)argv;
    /* Use hxhnt's embedded palette + ruleset for the terrain.  If
     * .hxseed is "fresh" (zero genome), rpg_init_map produces just
     * a uniform colour-0 map, but the binary still launches. */
    rpg_init_map(hx_embedded_genome);

    int px = RPG_MAP_W / 2;
    int py = RPG_MAP_H / 2;

    rpg_init_entities(px, py);
    rpg_player_init();
    ...
    while (1) {
        ...
        hl = sapp(hint, hl, "i=inv m=zap q=quit ");
        hint[hl] = 0;
        status(hint);
        fbflush();
