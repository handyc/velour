/* office40.c — Win95-style 17-app suite. Linux x86_64. No libc.
 *
 *   shell  notepad  word  mail  sheet  paint  hex  bfc  files
 *   find  calc  mines  ask   garden  hxhnt  rpg  lsys  bytebeat
 *
 * office40 gives every animal and NPC its own private wandering
 ...
 * 8000 Hz u8 mono into /tmp/office40_bb.raw and pipe to aplay via
...
/* office40 — meta-overworld coordinate stack.  rpg_world_pos[0] is
...
/* office40 NPC layer.  rpg_npc_at[i] = 0 → no NPC; otherwise the byte
...
static unsigned char rpg_npc_at[RPG_MAP_W * RPG_MAP_H];
static const unsigned char rpg_npc_pal[16] = {
    196, 202, 220, 226, 154, 118,  51,  39,
     33,  93, 201, 198, 252, 245, 240, 232,
};

/* office40 wander layer.  Each animal/NPC has a current step index
...
static unsigned char rpg_path_step[RPG_MAP_W * RPG_MAP_H];
static unsigned char rpg_path_id  [RPG_MAP_W * RPG_MAP_H];
...
static void rpg_init_map(const unsigned char *ruleset) {
    /* Random initial grid.  office40: re-seed from the world coord
...
static void rpg_init_entities(int spawn_x, int spawn_y) {
    ...
    for (int i = 0; i < RPG_MAP_W * RPG_MAP_H; i++) {
        ...
        /* office40 — sprinkle NPCs onto empty soil/sand at ~3% rate.
         * Hash byte non-zero (so the encoded head+body never collapses
         * to "no NPC"); high/low nibble pick distinct head/body colours. */
        if (!cat && (c == 1 || c == 2)) {
            unsigned q = hx_rand();
            if ((q & 31) == 0) {
...
/* office40 — generate a procedural closed-loop path of hex directions.
...
static void rpg_path_tick(int player_x, int player_y) {
    static unsigned char moved[RPG_MAP_W * RPG_MAP_H];
    mset(moved, 0, sizeof moved);
    unsigned long wseed = rpg_world_seed();
    for (int y = 0; y < RPG_MAP_H; y++) {
        for (int x = 0; x < RPG_MAP_W; x++) {
            int idx = y * RPG_MAP_W + x;
            ...
            int nx = x + dx, ny = y + dy;
            if (nx < 0 || nx >= RPG_MAP_W || ny < 0 || ny >= RPG_MAP_H) continue;
            int nidx = ny * RPG_MAP_W + nx;
...
/* ── bytebeat: tiny PCM synth (office40) ─────────────────
