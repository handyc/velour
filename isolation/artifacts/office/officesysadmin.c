...
#define APP_NAME    "officesysadmin"
#define APP_VERSION "sysad"
...
/* Pre-baked feature set for the `sysadmin` variant.
 * The OFFICE_FEATURE_* matrix below is hard-set to match
 * OfficeForge's same-named preset.  Each macro is still
 * #ifndef-guarded so you can override on the cc command line
 * (e.g. -DOFFICE_FEATURE_RPG=1) for one-off experiments. */
...
#define OFFICE_FEATURE_HXHNT 0
#endif
#ifndef OFFICE_FEATURE_RPG
#define OFFICE_FEATURE_RPG 0
#endif
...
int main_c(int argc, char **argv, char **envp) {
     ...
     * was added without updating the list. */
    {
        ...
        if (cmd[0] == 'o' && cmd[1] == 'f' && cmd[2] == 'f' &&
            ...
            /* Any suffix accepted — covers both numeric forks
             * (office42) and named variants (officegamer, etc.) */
            is_office_wrapper = 1;
        }
