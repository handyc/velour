/* office31.c — Win95-style 16-app suite. Linux x86_64. No libc.
 *
 *   shell  notepad  word  mail  sheet  paint  hex  bfc  files
 *   find  calc  mines  ask   garden  hxhnt  rpg  lsys
 *
 * office31 makes the rpg inner CAs *live*.  Press 'l' to toggle
...
static const struct hx_payload hx_embedded = {
    .pre     = "<<HXSEED-OPEN>>",
    /* xterm-256 cube codes whose RGB approximates water/grass/dirt/
     * lava — used as the rpg terrain RGB bases when no hxhnt.seed
     * has been written yet.  Override by evolving + saving in hxhnt. */
    .palette = { 26, 71, 131, 166 },
    .genome  = { 0 },
    .post    = "<<HXSEED-CLOSE>",
};
...
/* Returns 'q' = quit, 'g' = caller should run a GA. */
static int hx_display_seed(unsigned char *genome,
    ...
    for (long tick = 0; ; tick++) {
        ...
        if (flash_ttl > 0) {
            ...
            p = sapp(hint, p, "]  g=evolve r=palette d=save x=export q=quit ");
        }
        hint[p] = 0;
        status(hint);
        fbflush();
...
/* ── one GA session: mutate, score, breed, save winners ──────────── */
...
static void hx_run_ga(int pop, int gens, unsigned rseed) {
     ...
     * a key was pressed or not, so the loop ticks freely while ESC/q
     * still aborts.  Restored to blocking mode before return so the
     * post-GA winners screen and any caller paths get the regular
     * raw mode they expect. */
    struct ti t = term_orig;
    t.lflag &= ~(ICANON | ECHO);
    t.iflag &= ~(IXON | ICRNL);
    t.cc[6] = 0;     /* VMIN  */
    ...
    for (int gen = 0; gen < gens; gen++) {
        ...
        /* Non-blocking poll — generations advance whether or not the
         * user pressed anything; ESC/q still aborts. */
        unsigned char k[8];
        int n = read_key(k, sizeof k);
    ...
    /* Restore blocking mode for the winners screen. */
    t.cc[6] = 1;
    t.cc[5] = 2;
    io(0, TCSETS, &t);
    ...
    /* Adopt winner #1 as the live seed.  Persists to disk too so the
     * next office launch starts from this evolved point and rpg's
     * terrain RGBs reflect the new palette. */
    ...
    /* Winners screen: 1/2/3 splice-export the corresponding winner
     * into a runnable hxh-* binary.  q returns to display mode. */
    int seq = 1;
 ...
 * re-aggregate its 8×3 sample.  When a slot's owner mismatches the
 * world cell currently in that view position (player just moved),
 * re-seed the slot from the cell's deterministic hash.  When the
 * embedded ruleset is empty (collapses to uniform 0 in one step),
 * we re-seed every frame instead so the animation actually moves. */
static void rpg_animate_step(int px, int py, int rows_v) {
    ...
    for (int vy = 0; vy < rows_v; vy++) {
        for (int vx = 0; vx < RPG_VIS_W; vx++) {
            ...
            if (wx < 0 || wx >= RPG_MAP_W ||
                wy < 0 || wy >= RPG_MAP_H) {
                rpg_anim_init[slot] = 0;
                continue;
            }
            int reseed = !rpg_anim_init[slot]
            ...
            rpg_anim_aggregate(state,
                &rpg_cell_sample[(wy * RPG_MAP_W + wx) * 24]);
            rpg_cell_done[wy * RPG_MAP_W + wx] = 1;
        }
    }
}
...
struct EntityKind {
    ...
    unsigned char angle_steps; /* 1=45°, 2=90°, ... */
};

static const struct EntityKind rpg_kinds[] = {
    {0,   0, 0, 0, "none",  "",  "",                       0, 0},
    {'P', 5, 0, 1, "tree",  "F", "FF+[+F-F-F]-[-F+F+F]",   2, 1},
    {'P', 2, 0, 0, "bush",  "F", "F[+F]F[-F]F",            2, 1},
    {'B', 4, 0, 1, "house", "F", "F+F+F-F-F+F+F",          1, 2},
    {'B', 5, 0, 1, "tower", "F", "F+F-F-F+F+F+F-F",        2, 2},
    {'A', 2, 4, 0, "deer",  "F", "F[+F]F[-F]",             1, 1},
    {'A', 1, 2, 0, "bird",  "F", "F+F-F-F+F",              1, 1},
    {'I', 1, 0, 0, "chest", "F+F+F+F",  "F",               0, 2},
    {'I', 2, 0, 0, "gem",   "F+F+F+F+F+F", "F",            0, 1},
};
#define RPG_KIND_N ((int)(sizeof rpg_kinds / sizeof rpg_kinds[0]))

/* Per-kind sprite cache: rasterised+downsampled into a height×3 grid
 * of palette indices, plus a 4-entry palette derived from the rule
 * hash.  Built once at run_rpg startup. */
#define RPG_SPRITE_W 3
#define RPG_SPRITE_H_MAX 5
static unsigned char rpg_sprite[RPG_KIND_N][RPG_SPRITE_H_MAX * RPG_SPRITE_W];
static unsigned char rpg_sprite_pal[RPG_KIND_N][4];
static int rpg_sprites_ready;
...
/* Build the rasterised sprite + palette for one entity kind. */
static void rpg_sprite_build(int kind) {
    const struct EntityKind *e = &rpg_kinds[kind];
    if (!e->cat || !e->axiom || !*e->axiom) return;

    int len = 0;
    const char *cmds = lsys_expand_simple(e->axiom, e->rule ? e->rule : "F",
                                          e->iterations, &len);
    rpg_sprite_walk(cmds, len, e->angle_steps);
    ...
    /* Bucket the points into 3 cols × h rows. */
    ...
    /* Threshold each bucket → palette index 0/1/2/3. */
    unsigned max_hits = 1;
    for (int i = 0; i < RPG_SPRITE_W * h; i++)
        if (counts[i] > max_hits) max_hits = counts[i];
    unsigned char *out = rpg_sprite[kind];
    /* Zero the whole height-max strip first (rows above `h` stay 0). */
    for (int i = 0; i < RPG_SPRITE_W * RPG_SPRITE_H_MAX; i++) out[i] = 0;
    for (int i = 0; i < RPG_SPRITE_W * h; i++) {
        unsigned c = counts[i];
        int idx;
        if (c == 0)                       idx = 0;
        else if (c * 3 <= max_hits)       idx = 1;   /* sparse: light  */
        else if (c * 3 <= 2 * max_hits)   idx = 2;   /* mid           */
        else                              idx = 3;   /* dense: dark   */
        out[i] = (unsigned char)idx;
    }

    /* Derive 4-colour palette: pal[0] is "transparent" sentinel
     * (special-cased on render); pal[1..3] are 3 intensity shades
     * around the category's base RGB, perturbed by the rule hash so
     * each kind paints in a slightly different family. */
    unsigned long hh = 0xcbf29ce484222325UL;
    hh = rpg_str_hash(hh, e->axiom);
    hh = rpg_str_hash(hh, e->rule ? e->rule : "");
    hh ^= ((unsigned long)e->iterations * 0x9e3779b97f4a7c15UL);
    hh ^= ((unsigned long)e->angle_steps * 0xc2b2ae3d27d4eb4fUL);
    int dr = (int)((hh      ) & 0x3f) - 32;
    int dg = (int)((hh >>  8) & 0x3f) - 32;
    int db = (int)((hh >> 16) & 0x3f) - 32;
    const struct RpgCatBase *cb = &rpg_cat_base_table[rpg_cat_index(e->cat)];
    int br = (int)cb->r + dr;
    int bg = (int)cb->g + dg;
    int bb = (int)cb->b + db;
    rpg_sprite_pal[kind][0] = 0;     /* sentinel — never emitted */
    /* Light highlight (sparse foliage): blend toward white. */
    rpg_sprite_pal[kind][1] = (unsigned char)rpg_rgb_to_xterm256(
        clamp255(br + (255 - br) * 55 / 100),
        clamp255(bg + (255 - bg) * 55 / 100),
        clamp255(bb + (255 - bb) * 55 / 100));
    /* Mid: base. */
    rpg_sprite_pal[kind][2] = (unsigned char)rpg_rgb_to_xterm256(
        clamp255(br), clamp255(bg), clamp255(bb));
    /* Dense (trunk / shadow): darker. */
    rpg_sprite_pal[kind][3] = (unsigned char)rpg_rgb_to_xterm256(
        clamp255(br * 45 / 100),
        clamp255(bg * 45 / 100),
        clamp255(bb * 45 / 100));
}
...
static void rpg_render_view(int px, int py) {
    ...
    for (int vy = 0; vy < rows_v; vy++) {
        ...
        for (int vx = 0; vx < RPG_VIS_W; vx++) {
            ...
            if (!kind) continue;
            const struct EntityKind *e = &rpg_kinds[kind];
            const unsigned char *sprite = rpg_sprite[kind];
            const unsigned char *spal   = rpg_sprite_pal[kind];
            int base_x = origin_x + row_offset + vx * RPG_CELL_W;
            int base_y = origin_y + vy * RPG_CELL_H;
...
static int run_rpg(int argc, char **argv) {
    ...
    while (1) {
        ...
        hl = sapp(hint, hl, "wadezx=move i=inv m=zap l=live q=quit ");
        hint[hl] = 0;
        status(hint);
        fbflush();
