/* office18.c — Win95-style 14-app suite. Linux x86_64. No libc.
 *
 *   shell  notepad  word  mail  sheet  paint  hex  bfc  files
 *   find  calc  mines  ask   garden  hxhnt
 *
 * hxhnt's exported binary is now the entire office18 program with
...
/* Splice export: read the running ELF, find the .hxseed OPEN marker,
 * overwrite the 4-byte palette and 4096-byte genome at fixed offsets
 * inside the payload, write the result to `dst`.  The CLOSE marker
 * after the genome is sanity-checked but not modified.
 *
 * Designed parameterically so office19+ can use the same call-shape
 * for garden's exports — they'll just supply their own marker pair
 * and payload bytes. */
static int hx_splice_export(const char *dst,
    ...
    long off = hx_find_marker(hx_export_buf, total,
                              hx_payload_pre, sizeof hx_payload_pre);
    if (off < 0) return -1;

    long pal_off    = off + (long)sizeof hx_payload_pre;
    long genome_off = pal_off + HX_PAL_BYTES;
    long post_off   = genome_off + HX_GBYTES;
    if (post_off + (long)sizeof hx_payload_post > total) return -1;

    /* Verify the CLOSE marker is where we expect — guards against
     * the linker reordering the .hxseed contents between forks. */
    if (mcmp(hx_export_buf + post_off, hx_payload_post,
             sizeof hx_payload_post) != 0) return -1;

    mcpy(hx_export_buf + pal_off,    pal,    HX_PAL_BYTES);
    mcpy(hx_export_buf + genome_off, genome, HX_GBYTES);
