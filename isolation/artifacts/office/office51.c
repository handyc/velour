/* office51.c -- Win95-style 12-app suite. Linux x86_64. No libc.
 *
 *   shell  notepad  sheet  hex  files  calc
 *   ask   garden  hxhnt  rpg  lsys  bytebeat  saver
 *
 * office51 — three changes, all in rpg's mosaic-shift path so the
 * player never sees an "edge of the world" loading event.  Pushes
 * the binary back over the 64 KB cap (66 KB) — a follow-up fork can
 * trim or split if the size matters more than the smoothness.
 *
