/* office34.c — Win95-style 16-app suite. Linux x86_64. No libc.
 *
 *   shell  notepad  word  mail  sheet  paint  hex  bfc  files
 *   find  calc  mines  ask   garden  hxhnt  rpg  lsys
 *
 * office34 fixes the live-view edge garbling and adds a bending
...
static int hx_display_seed(unsigned char *genome,
    ...
    for (long tick = 0; ; tick++) {
        ...
        if (flash_ttl > 0) {
            ...
            p = sapp(hint, p, "  g=ga h=hunt []=mut r=pal d=save x=export q=quit ");
        }
        hint[p] = 0;
        status(hint);
        fbflush();
 ...
 * inv_count is the running total for status-bar display.  bend_skill
 * tracks the player's mastery of each terrain (0..9), levelling up
 * with every successful bend. */
struct RpgPlayer {
    int hp, max_hp;
    int mp, max_mp;
    unsigned char inv[RPG_KIND_N];
    int inv_count;
    unsigned char bend_skill[4];   /* one per terrain */
};
...
static int rpg_bend(int t, char *msg) {
    ...
    if (gens < 1) {
        int n = sapp(msg, 0, "not enough mana to bend ");
        n = sapp(msg, n, rpg_terrain_name[t]);
        msg[n] = 0;
        return 0;
    }
    if (gens > 10) gens = 10;
...
static int run_rpg(int argc, char **argv) {
    ...
    while (1) {
        ...
        hl = sapp(hint, hl, "wadezx=move i=inv m=zap l=live k=speeds 0..3=bend q=quit ");
        hint[hl] = 0;
        status(hint);
        fbflush();
        ...
        if (k[0] >= '0' && k[0] <= '3') {
            ...
            action[0] = 0;
            rpg_bend(t, action);
            /* The inline GA may have left polling termios installed;
             * re-assert whatever the rpg loop should be in. */
            rt.cc[6] = rpg_animating ? 0 : 1;
            rt.cc[5] = rpg_animating ? 1 : 0;
            io(0, TCSETS, &rt);
            idle_ticks = 0;
