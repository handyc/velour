/* office56.c -- Win95-style 11-app suite. Linux x86_64. No libc.
 *
 *   shell  notepad  sheet  hex  files  calc
 *   ask   garden  hxhnt  rpg  lsys  saver
 *
 * office56 — sheet gains two combine ops that never error on
 * dimension mismatch: M (zero-pad multiply) and K (Kronecker
 * product).  Both write to sheet C and switch focus there; status
 * banner shows the dims used + any padding/clipping.
 *
