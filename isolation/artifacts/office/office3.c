/* office3.c — Win95-style 9-app suite. Linux x86_64. No libc.
 *
 *   shell  notepad  word  mail  sheet  paint  hex  bfc  files
 *
 * Adds on top of office2.c:
 *   - hex: tab toggles to ASCII pane, printable keys overwrite there
 *   - sheet: SUM/AVG/MIN/MAX(A1:B5) ranges on top of arithmetic
 *   - paint: canvas.txt now persists per-cell colour (round-trips)
 *   - word: ^J reflows the current paragraph to fit the screen
 *   - files: new app — directory browser, enter opens in notepad
 */
...
static int run_hex(int argc, char **argv) {
    ...
    while (1) {
        ...
        if (k[0] == '\t') { ascii_pane = !ascii_pane; nibhi = 1; continue; }
        if (n >= 3 && k[0] == 0x1b && k[1] == '[') {
            switch (k[2]) {
