/* office6.c — Win95-style 12-app suite. Linux x86_64. No libc.
 *
 *   shell  notepad  word  mail  sheet  paint  hex  bfc  files
 *   find  calc  mines
 *
 * Same apps + fixes as office5. One additional fix:
 *   - When you arrow left/right between menus while one is open,
 *     the previous menu's pulldown is now erased before the new one
 *     draws. office5 was leaving the old pulldown on screen, so the
 *     user could see two menus at once during navigation. The fix is
 *     a single body-area teal-fill at the top of each menu_run
 *     iteration, before the new pulldown is drawn.
 */
