/* office50.c -- Win95-style 12-app suite. Linux x86_64. No libc.
 *
 *   shell  notepad  sheet  hex  files  calc
 *   ask   garden  hxhnt  rpg  lsys  bytebeat  saver
 *
 * office50 — two changes:
 *
 *   1. Per-overworld base palette in rpg.  Each of the 9 sub-
 *      overworlds in the 192×192 mosaic now derives its own 4-colour
 *      palette (rock/sand/soil/water archetypes ± per-seed jitter)
 *      from the same world seed that drives terrain layout.  When
 *      the mosaic shifts, all 9 palettes refresh.  Neighbouring
 *      overworlds are visibly distinct, so crossing the boundary
 *      reads as walking into a new biome.
 *
 *   2. Cap max_tokens=1024 in Ask.  Anthropic always required it;
 *      for OpenAI / proxies it was implicit — the pekpik proxy
 *      defaults to a very large per-call budget for GPT-5.5 keys,
 *      eating the whole rate-limit on every send.  Now we send
 *      max_tokens with the request body for all three providers.
 *
 ...
 * world seeds.  Called whenever the mosaic loads or shifts. */
