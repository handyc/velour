/* office32.c — Win95-style 16-app suite. Linux x86_64. No libc.
 *
 *   shell  notepad  word  mail  sheet  paint  hex  bfc  files
 *   find  calc  mines  ask   garden  hxhnt  rpg  lsys
 *
 * office32 reworks the rpg terrain semantics + animation rates:
 *
...
static void rpg_animate_step(int px, int py, int rows_v) {
    ...
    for (int vy = 0; vy < rows_v; vy++) {
        for (int vx = 0; vx < RPG_VIS_W; vx++) {
            ...
            int terrain = rpg_map[wy * RPG_MAP_W + wx] & 3;
...
/* Pick an entity kind for a cell, biased by its terrain type.  r is
 * 0..255 from the same RNG as rpg_init_map.  Terrain meanings:
 *   0 rock     buildings + items, no plants
 *   1 sand     trees, bushes, animals (no buildings)
 *   2 soil     all kinds — buildings, plants, animals, items
 *   3 water    just animals (waterfowl etc.) */
static int rpg_kind_for(int colour, unsigned r) {
    switch (colour & 3) {
    case 0:                          /* rock — buildings + items */
        if (r <  10) return 3;       /* house  */
        if (r <  18) return 4;       /* tower  */
        if (r <  22) return 7;       /* chest  */
        if (r <  28) return 8;       /* gem    */
        return 0;
    case 1:                          /* sand — plants + animals */
        if (r <  15) return 1;       /* tree   */
        if (r <  30) return 2;       /* bush   */
        if (r <  38) return 5;       /* deer   */
        if (r <  43) return 6;       /* bird   */
        return 0;
    case 2:                          /* soil — everything */
        if (r <  12) return 1;       /* tree   */
        if (r <  22) return 2;       /* bush   */
        if (r <  28) return 3;       /* house  */
        if (r <  32) return 4;       /* tower  */
        if (r <  40) return 5;       /* deer   */
        if (r <  46) return 6;       /* bird   */
        if (r <  50) return 7;       /* chest  */
        return 0;
    case 3:                          /* water — birds only */
        if (r <  12) return 6;       /* bird   */
        return 0;
    }
    return 0;
}
...
static int run_rpg(int argc, char **argv) {
    ...
    while (1) {
        ...
        hl = sapp(hint, hl, "wadezx=move i=inv m=zap l=live k=speeds q=quit ");
        hint[hl] = 0;
        status(hint);
        fbflush();
