/* supercell.c -- focused 9-app fork of office64. Linux x86_64. No libc.
 *
 *   shell  word(=notepad)  sheet(+calc merged)  hex  files
 *   ask   prompt           hxhnt              rpg  lsys
 *
 * supercell strips office64's network stack (net/http/echo/finger/
 * gopher/probe/dns/ftp/sshtel), the screensaver, and the standalone
 * theme garden.  The CA/L-system breeding that lived in `garden` is
 * still embedded in `rpg`'s textures and `lsys`'s viewer; the GA
 * primitives are still the engine behind `hxhnt`'s rule evolver.
 *
 * Calc has been merged into sheet — typing `=2^32` in any cell
 * evaluates with the same 64-bit feval_* chain calc used.  `calc`
 * is kept as an alias for `sheet` from both the home shell and the
 * basename dispatcher.  `word` is an alias for `notepad`.
 *
 * New: `prompt` is a 4 KB system-prompt slot persisted to
 * `~/.supercell_prompt.bin`.  Run `prompt` to edit it; `ask` then
 * prepends it as the system message of every chat completion, so
 * the user can give the LLM stable context across sessions
 * (project description, tone preferences, lab setup, etc.) without
 * retyping.
 *
...
static int run_shell(int argc, char **argv) {
    ...
    while (running) {
        ...
        body_at(2, 3, "Welcome to Supercell. Built-in commands:", SCREEN_W - 4);
        body_at(2, 4, "  word  sheet  hex  files",
                SCREEN_W - 4);
        body_at(2, 5, "  ask  prompt  hxhnt  rpg  lsys  exit",
                SCREEN_W - 4);
        body_at(2, 6, "  (word=notepad; sheet has =EXPR cells; prompt edits a 4 KB",
                SCREEN_W - 4);
        body_at(2, 7, "   system prompt; Alt+F / F10 opens menus in every app)",
                SCREEN_W - 4);
        if (msg[0]) {
...
/* ── 4 KB system-prompt slot (supercell only) ──────────────
 * Persisted to ./prompt.bin — `prompt` edits it, `ask` prepends
 * it as the system message of every chat completion.  Reads
 * lazily (just before each request) so the file can be edited
 * by an external editor between turns. */
#define PROMPT_MAX 4096
static char g_llm_prompt[PROMPT_MAX];
static int  g_llm_prompt_len = 0;

static void prompt_load(void) {
    g_llm_prompt_len = 0;
    int fd = (int)op("prompt.bin", O_RDONLY, 0);
    if (fd < 0) return;
    long n = rd(fd, g_llm_prompt, PROMPT_MAX - 1);
    cl(fd);
    if (n < 0) n = 0;
    g_llm_prompt_len = (int)n;
    /* Trim trailing whitespace (newlines from a notepad save) so we
     * don't send "\n\n\n" as a system prompt and confuse providers. */
    while (g_llm_prompt_len > 0) {
        char c = g_llm_prompt[g_llm_prompt_len - 1];
        if (c == ' ' || c == '\n' || c == '\r' || c == '\t')
            g_llm_prompt_len--;
        else break;
    }
...
/* ── prompt: edit the 4 KB system prompt slot ──────────────
 * Uses the shared notepad UI (full arrow-key navigation +
 * menu bar) but pinned to ./prompt.bin and capped at 4 KB
 * on save.  Anything past PROMPT_MAX-1 is silently truncated
 * — the editor still lets you keep typing, you just won't
 * see those bytes echoed back to the LLM. */
static int run_prompt(int argc, char **argv) {
    (void)argc; (void)argv;
    current_ms = &ms_notepad;
    /* Load existing prompt.bin into the shared notepad buffer. */
    int fd = (int)op("prompt.bin", O_RDONLY, 0);
    if (fd >= 0) {
        blen = (int)rd(fd, buf, BUF_CAP - 1);
        if (blen < 0) blen = 0;
    ...
    /* Pin save target so MA_SAVE writes back to prompt.bin even if
     * the user opened with no path. */
    const char *fn = "prompt.bin";
    int j = 0;
    while (fn[j] && j < (int)sizeof fname - 1) { fname[j] = fn[j]; j++; }
    fname[j] = 0;
    bcur = 0; btop = 0;
    int rc = notepad_loop("System Prompt (4 KB cap)");
    /* On exit, truncate the on-disk file to PROMPT_MAX-1 bytes so
     * ask doesn't read a giant prompt that overflows the request
     * buffer.  Refresh g_llm_prompt so the next ask call sees the
     * latest version without re-loading from disk. */
    if (blen > PROMPT_MAX - 1) {
        int fd2 = (int)op("prompt.bin",
                          O_WRONLY | O_CREAT | O_TRUNC, 0644);
        if (fd2 >= 0) {
            wr(fd2, buf, (size_t)(PROMPT_MAX - 1));
            cl(fd2);
        }
    }
    prompt_load();
...
int main_c(int argc, char **argv, char **envp) {
    ...
    const char *cmd = (argc > 0) ? basename_(argv[0]) : "supercell";
    int sub_argc = argc;
    char **sub_argv = argv;
    /* If cmd is "supercell" (or any officeN wrapper, kept for jail.c
     * parity), peel argv[0] and treat argv[1] as the subcommand. */
    {
        int is_wrapper = 0;
        if (scmp(cmd, "supercell") == 0) is_wrapper = 1;
