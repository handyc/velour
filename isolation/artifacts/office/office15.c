/* office15.c — Win95-style 14-app suite. Linux x86_64. No libc.
 *
 *   shell  notepad  word  mail  sheet  paint  hex  bfc  files
 *   find  calc  mines  ask   garden  hxhnt
 *
 * Adds hxhnt — a port of isolation/artifacts/oneclick_class4/hunter.c
 * as a 14th app.  Class-4 hex-CA hunter: animates a packed 4096-byte
 * genome on a 14×14 hex grid, evolves new genomes via tournament-2
 * GA on a fitness function that rewards activity in the edge-of-chaos
 * band (target ~0.12 changes/cell tail-averaged).  Self-replication
 * is preserved via *seed files* rather than self-modifying binaries:
 *
 *   ./hxhnt.seed                  (current seed: 4 magic + 4 palette + 4096 genome)
 *   ./hxhnt_winner_N.seed         (each evolved winner)
 *
 * From the office shell:
 *   hxhnt              # display mode: animate the seed for 10 ticks
 *   hxhnt POP GENS     # GA: evolve POP agents over GENS generations
 *
 * All the helpers (mcpy, fbs, sgrbgfg, raw mode, etc.) are reused
 * from office's existing kit; the hxhnt-specific code is just the
 * stepping kernel, fitness function, GA ops, and palette logic.
 *
...
static int run_hxhnt(int argc, char **argv) {
    ...
    body_clear();
    body_at(2, 3, "Top winners written as hxhnt_winner_N.seed:", SCREEN_W - 4);
