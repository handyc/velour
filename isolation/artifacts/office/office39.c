/* office39.c — Win95-style 17-app suite. Linux x86_64. No libc.
 *
 *   shell  notepad  word  mail  sheet  paint  hex  bfc  files
 *   find  calc  mines  ask   garden  hxhnt  rpg  lsys  bytebeat
 *
 * office39 adds a tiny bytebeat synth.  Five preset PCM formulas
 * (Crowd, 42 Melody, Three Note, Skyline, Phaser); generate 4 s of
 * 8000 Hz u8 mono into /tmp/office39_bb.raw and pipe to aplay via
...
/* office39 — meta-overworld coordinate stack.  rpg_world_pos[0] is
...
/* office39 NPC layer.  rpg_npc_at[i] = 0 → no NPC; otherwise the byte
...
static void rpg_init_map(const unsigned char *ruleset) {
    /* Random initial grid.  office39: re-seed from the world coord
...
static void rpg_init_entities(int spawn_x, int spawn_y) {
    ...
    for (int i = 0; i < RPG_MAP_W * RPG_MAP_H; i++) {
        ...
        /* office39 — sprinkle NPCs onto empty soil/sand at ~3% rate.
...
/* ── bytebeat: tiny PCM synth (office39) ─────────────────
