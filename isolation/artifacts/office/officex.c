/* officex.c -- core productivity + xpg (rpg+hxhnt+lsys merged).
 * Linux x86_64. No libc.
 *
 *   shell  word(=notepad)  sheet(+calc merged)  hex  files  xpg
 *
 * officex is supercell minus the LLM client (ask + prompt) minus
 * the standalone hxhnt and lsys menu entries — those last two are
 * folded into xpg, which is the single tile-explorer-with-evolved-
...
int main_c(int argc, char **argv, char **envp) {
    ...
    const char *cmd = (argc > 0) ? basename_(argv[0]) : "officex";
    int sub_argc = argc;
    char **sub_argv = argv;
    /* If cmd is "officex" / "supercell" (or any officeN wrapper, kept
     * for jail.c parity), peel argv[0] and treat argv[1] as the
     * subcommand. */
    {
        int is_wrapper = 0;
        if (scmp(cmd, "officex") == 0 || scmp(cmd, "supercell") == 0)
            is_wrapper = 1;
