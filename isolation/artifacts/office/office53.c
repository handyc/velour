/* office53.c -- Win95-style 12-app suite. Linux x86_64. No libc.
 *
 *   shell  notepad  sheet  hex  files  calc
 *   ask   garden  hxhnt  rpg  lsys  bytebeat  saver
 *
 * office53 — silent terrain bends in rpg.  Pressing 0/1/2/3 used to
 * hand the screen over to hxhnt's progress paint (population /
 * generation / fitness / activity tail bars) for the duration of the
 * GA session, then snap back to rpg.  Now hx_run_ga_session takes a
 * `silent` flag: when set, it skips the polling-termios switch, the
 * per-generation hx_paint_progress + read_key, and the abort-on-q
 * check.  The GA still runs the same number of generations and still
 * adopts the winner; rpg_bend invalidates the cell cache + refreshes
 * palettes as before, so the world repopulates with the evolved CA
 * on the very next render — the player just sees a brief pause, then
 * the overworld shifts colour and pattern under their feet.  The
 * existing hxhnt callers (run_continuous_hunt, hx_run_ga) pass
 * silent=0 so their interactive view is unchanged.
 *
 ...
 * 8000 Hz u8 mono into /tmp/office50_bb.raw and pipe to aplay via
...
/* ── bytebeat: tiny PCM synth (office50) ─────────────────
