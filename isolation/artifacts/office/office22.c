/* office22.c — Win95-style 16-app suite. Linux x86_64. No libc.
 *
 *   shell  notepad  word  mail  sheet  paint  hex  bfc  files
 *   find  calc  mines  ask   garden  hxhnt  rpg  lsys
 *
 * office22 deepens rpg into a real little game.  The 64×64 terrain
 ...
 *   1. terrain — same staggered hex blocks as office20.
...
static void rpg_render_view(int px, int py, const unsigned char *pal) {
    int origin_y = 1;
    ...
    for (int vy = 0; vy < rows_v; vy++) {
        ...
        for (int vx = 0; vx < RPG_VIS_W; vx++) {
            ...
            int in_map = (wx >= 0 && wx < RPG_MAP_W &&
                          wy >= 0 && wy < RPG_MAP_H);
            if (in_map) colour = pal[rpg_map[wy * RPG_MAP_W + wx]];
    ...
    for (int vy = 0; vy < rows_v; vy++) {
        ...
        for (int vx = 0; vx < RPG_VIS_W; vx++) {
            ...
            int kind = rpg_kind_at[wy * RPG_MAP_W + wx];
            if (!kind) continue;
            const struct EntityKind *e = &rpg_kinds[kind];
            unsigned char terrain_col = pal[rpg_map[wy * RPG_MAP_W + wx]];
            int base_x = origin_x + row_offset + vx * RPG_CELL_W;
            int base_y = origin_y + vy * RPG_CELL_H;
            int art_x = base_x + (RPG_CELL_W - 3) / 2;   /* centre 3-wide */
            int h = e->height;
            for (int li = 0; li < h; li++) {       /* li = 0 → bottom */
                int art_row = h - 1 - li;
                int sy = base_y + 2 - li;          /* bottom of cell + grow up */
    ...
    if (px >= 0 && px < RPG_MAP_W && py >= 0 && py < RPG_MAP_H)
        colour_under = pal[rpg_map[py * RPG_MAP_W + px]];
...
static int run_rpg(int argc, char **argv) {
    ...
    while (1) {
        paint_desktop();
        chrome("rpg");
        rpg_render_view(px, py, hx_embedded_palette);
