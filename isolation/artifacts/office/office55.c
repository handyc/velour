/* office55.c -- Win95-style 11-app suite. Linux x86_64. No libc.
 *
 *   shell  notepad  sheet  hex  files  calc
 *   ask   garden  hxhnt  rpg  lsys  saver
 *
 * office55 — sheet gains 3 tabs (A · B · C) and an M-hotkey matrix
 * multiply.  The cell store grows from cell[12][8][16] to
 * cell[3][12][8][16] (BSS only — no file-size impact).  A new
 * `cur_sheet` int picks which sheet is on screen; Tab / 1 / 2 / 3
 * cycle between A, B, C.  Pressing M evaluates sheets A and B as
 * matrices: each sheet's dimensions are auto-detected from its
 * filled rectangular region (max-filled-row+1 × max-filled-col+1,
 * empty cells treated as 0).  The multiply rejects empty inputs
 * and dimension mismatches with a status-line error showing the
 * dims it found; on success it writes the m×p product into sheet
 * C, switches focus to C, and displays "C = A·B  m×n · n×p → m×p".
 * Cell values that begin with '=' are evaluated through the
 * existing feval_expr (so formulas in A or B still work as
 * matrix entries).  Tab strip at row 1 shows `[A] B  C` style
 * with the active sheet inverted.  Save/open still scoped to the
 * active sheet — the CSV format is unchanged so files written by
 * office54's sheet load cleanly into any tab.
 *
...
static int read_int_in_sheet(int s, int r, int c) {
    int saved = cur_sheet;
    cur_sheet = s;
    const char *t = cell[s][r][c];
    int v = (t[0] == '=') ? sheet_eval(t) : parse_int_literal(t);
    cur_sheet = saved;
    return v;
}
static void sheet_multiply(void) {
    ...
    if (ac != br) {
        n = sapp(sheet_msg, 0, " mismatch A.c=");
        n = sheet_app_int(sheet_msg, n, ac);
        n = sapp(sheet_msg, n, " B.r=");
    ...
    for (int i = 0; i < ar; i++) {
        for (int j = 0; j < bc; j++) {
            long acc = 0;
            for (int k = 0; k < ac; k++) {
                acc += (long)read_int_in_sheet(0, i, k)
                     * (long)read_int_in_sheet(1, k, j);
            }
            char tmp[16];
            int  tn = itoa_((int)acc, tmp);
            if (tn > 15) tn = 15;
            mcpy(cell[2][i][j], tmp, tn);
            cell[2][i][j][tn] = 0;
        }
    }
...
static int run_sheet(int argc, char **argv) {
    ...
    while (1) {
        ...
        for (int r = 0; r < SHEET_ROWS && r + 3 < SCREEN_H - 1; r++) {
            ...
            for (int c = 0; c < SHEET_COLS; c++) {
                ...
                if (is_formula && !(editing && sel)) {
                    int v = sheet_eval(cell[cur_sheet][r][c]);
                    len = itoa_(v, shown);
        ...
        const char *h = editing
            ...
            : "  arrows | e edit | tab/1-3 sheet | M multiply A.B->C | s save | q back";
