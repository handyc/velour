...
static int run_shell(int argc, char **argv) {
    ...
    while (running) {
        ...
        body_at(2, 5, "  ask  garden  hxhnt  rpg  lsys",
                SCREEN_W - 4);
        body_at(2, 6, "  saver  net  http  echo  finger  gopher  probe  dns  ftp  exit",
                SCREEN_W - 4);
        ...
        if (k[0] == '\r' || k[0] == '\n') {
            ...
            else if (scmp(cmd, "net") == 0)    rc = run_net      (sub_argc, sub_argv);
            else if (scmp(cmd, "http") == 0)   rc = run_http     (sub_argc, sub_argv);
            else if (scmp(cmd, "echo") == 0)   rc = run_echo     (sub_argc, sub_argv);
            else if (scmp(cmd, "finger") == 0) rc = run_finger   (sub_argc, sub_argv);
            else if (scmp(cmd, "gopher") == 0) rc = run_gopher   (sub_argc, sub_argv);
            else if (scmp(cmd, "probe") == 0)  rc = run_probe    (sub_argc, sub_argv);
            else if (scmp(cmd, "dns") == 0)    rc = run_dns      (sub_argc, sub_argv);
            else if (scmp(cmd, "ftp") == 0)    rc = run_ftp      (sub_argc, sub_argv);
