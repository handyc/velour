/* office16.c — Win95-style 14-app suite. Linux x86_64. No libc.
 *
 *   shell  notepad  word  mail  sheet  paint  hex  bfc  files
 *   find  calc  mines  ask   garden  hxhnt
 *
...
static int run_shell(int argc, char **argv) {
    ...
    while (running) {
        ...
        body_at(2, 4, "  notepad  word  mail  sheet  paint  hex  bfc",
                SCREEN_W - 4);
        body_at(2, 5, "  files  find  calc  mines  ask  garden  hxhnt  exit",
                SCREEN_W - 4);
...
/* Does the running binary already carry a TAIL?  Used to decide
 * whether to copy bytes-minus-tail when re-exporting (so we don't
 * pile multiple tails onto one binary as winners cascade through
 * generations of evolution). */
static int hx_self_has_tail(void) {
    int fd = (int)op("/proc/self/exe", O_RDONLY, 0);
    if (fd < 0) return 0;
    /* Seek to the start of where the tail would be. */
    long sz = sys3(SYS_lseek, fd, 0, 2 /* SEEK_END */);
    if (sz < HX_TAIL_BYTES) { cl(fd); return 0; }
    sys3(SYS_lseek, fd, sz - HX_TAIL_BYTES, 0 /* SEEK_SET */);
    char magic[HX_MAGIC_BYTES];
    int has = (rd(fd, magic, HX_MAGIC_BYTES) == HX_MAGIC_BYTES)
           && (mcmp(magic, HX_TAIL_MAGIC, HX_MAGIC_BYTES) == 0);
    cl(fd);
    return has;
}

/* Read a seed by extracting it from /proc/self/exe's tail.  Returns
 * 0 on success, -1 if the running binary has no tail. */
static int hx_read_self_tail(unsigned char *pal, unsigned char *genome) {
    int fd = (int)op("/proc/self/exe", O_RDONLY, 0);
    if (fd < 0) return -1;
    long sz = sys3(SYS_lseek, fd, 0, 2);
    if (sz < HX_TAIL_BYTES) { cl(fd); return -1; }
    sys3(SYS_lseek, fd, sz - HX_TAIL_BYTES, 0);
    char magic[HX_MAGIC_BYTES];
    int ok = (rd(fd, magic,  HX_MAGIC_BYTES) == HX_MAGIC_BYTES)
          && (mcmp(magic, HX_TAIL_MAGIC, HX_MAGIC_BYTES) == 0)
          && (rd(fd, pal,    HX_PAL_BYTES)   == HX_PAL_BYTES)
...
/* Export the running office binary plus the given pal+genome tail to
 * a new file at `dst`.  Re-exports strip our own tail before copying
 * so a chain of evolved exports doesn't accumulate stale tails. */
static int hx_export_binary(const char *dst,
                            const unsigned char *pal,
                            const unsigned char *genome) {
    int self_tailed = hx_self_has_tail();
    int s = (int)op("/proc/self/exe", O_RDONLY, 0);
    if (s < 0) return -1;
    long sz = sys3(SYS_lseek, s, 0, 2);
    if (sz < 0) { cl(s); return -1; }
    sys3(SYS_lseek, s, 0, 0);
    long engine_sz = self_tailed ? sz - HX_TAIL_BYTES : sz;
    ...
    while (left > 0) {
        ...
        long got = rd(s, buf, want);
        if (got <= 0) break;
        long off = 0;
        while (off < got) {
            long k = wr(d, buf + off, got - off);
            if (k <= 0) { cl(s); cl(d); return -1; }
            off += k;
        }
        left -= got;
    }
    wr(d, HX_TAIL_MAGIC, HX_MAGIC_BYTES);
    wr(d, pal,           HX_PAL_BYTES);
    wr(d, genome,        HX_GBYTES);
    cl(s); cl(d);
...
/* Display mode: paint a chrome, then animate the seed for `ticks`
 * frames.  ESC or 'q' quits early. */
static void hx_display_seed(const unsigned char *genome,
    ...
    int gx = 4, gy = 3;     /* leave a border + chrome above */
    paint_desktop();
    chrome("hxhnt");
    body_clear();
    for (int t = 0; t <= ticks; t++) {
        ...
        p = sapp(hint, p, " tick ");
        p += utoa((unsigned)t,    hint + p);
        p = sapp(hint, p, "/");
        p += utoa((unsigned)ticks, hint + p);
        p = sapp(hint, p, "  pal=[");
        ...
        hx_step_grid(genome, hx_grid_a, hx_grid_b);
        mcpy(hx_grid_a, hx_grid_b, sizeof hx_grid_a);
        /* Sleep 150 ms but allow ESC/q to break early. */
        unsigned char k[8];
        long start = time_();
        while (time_() - start < 1) {
            int n = read_key(k, sizeof k);
            if (n > 0 && (k[0] == 0x1b || k[0] == 'q' || k[0] == 'Q'))
                return;
            if (n == 0) hx_sleep_ms(150);
            if (n == 0) break;
        }
    }
    /* Wait for any key before returning to shell. */
    unsigned char k[8];
    read_key(k, sizeof k);
}
...
static int run_hxhnt(int argc, char **argv) {
    ...
    /* Bootstrap order: prefer the running binary's own tail (if it
     * was launched as an exported hxh-* binary), fall back to the
     * shared ./hxhnt.seed, fall back to a fresh random palette +
     * identity genome.  Same precedence as the original hunter. */
    if (hx_read_self_tail(hx_seed_pal, hx_seed_genome) != 0 &&
        hx_read_seed(HX_SEED_FILE, hx_seed_pal, hx_seed_genome) != 0) {
        hx_invent_palette(hx_seed_pal);
        hx_identity_genome(hx_seed_genome);
    ...
    /* Display mode: animate seed on a fresh random grid for 10 ticks. */
    if (mode <= 0) {
        hx_display_seed(hx_seed_genome, hx_seed_pal,
                        (unsigned int)time_(), 10);
        term_cooked();
        return 0;
    }
    ...
    body_clear();
    body_at(2, 3, "Top winners exported as runnable binaries + seed files:",
            SCREEN_W - 4);

    int next_slot = 1;
    for (int w = 0; w < HX_WINNERS && w < pop; w++) {
        /* .seed companion (used for re-load inside office). */
        char path[32];
        for (;;) {
            int p = sapp(path, 0, "hxhnt_winner_");
            p += utoa((unsigned)next_slot, path + p);
            p = sapp(path, p, ".seed");
            path[p] = 0;
            int fd = (int)op(path, O_RDONLY, 0);
            if (fd < 0) break;
        ...
        hx_write_seed(path, hx_pals[w], hx_pool[w]);

        /* Functional self-replicating binary export. */
        char export_name[HX_EXPORT_NAME_LEN + 1];
        hx_make_export_name(export_name, w + 1);
        hx_export_binary(export_name, hx_pals[w], hx_pool[w]);

        /* Status line: fitness + binary export name + 4-swatch palette. */
        char line[80]; int p = 0;
        p = sapp(line, p, "  #");
        p += utoa((unsigned)(w + 1), line + p);
        p = sapp(line, p, "  fit=");
        long ff = (long)(hx_fit[w] * 100.0 + 0.5);
        p += utoa((unsigned)(ff / 100), line + p); line[p++] = '.';
        p += u2((unsigned)(ff % 100), line + p);
        p = sapp(line, p, "  ");
        p = sapp(line, p, export_name);
        line[p] = 0;
        body_at(2, 5 + w, line, SCREEN_W - 4);

        /* Palette swatches lined up at the right edge. */
        int sx = SCREEN_W - 10;
        if (sx > 4) {
            cup(sx, 5 + w);
            for (int i = 0; i < HX_K; i++) {
     ...
     * runs animate the evolved winner. */
    hx_write_seed(HX_SEED_FILE, hx_pals[0], hx_pool[0]);

    status(" any key to return ");
    fbflush();
    unsigned char k[8];
    read_key(k, sizeof k);
