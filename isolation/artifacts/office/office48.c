/* office48.c — Win95-style 18-app suite. Linux x86_64. No libc.
 *
 *   shell  notepad  word  mail  sheet  paint  hex  bfc  files
 *   find  calc  mines  ask   garden  hxhnt  rpg  lsys  bytebeat
 *   saver
 *
 * office48 makes the `r`-key key-grabber also pull the model name
 ...
 *     into /tmp/office48_keys.txt, scan for "sk-" tokens, and
 ...
 * 8000 Hz u8 mono into /tmp/office48_bb.raw and pipe to aplay via
...
/* office48 — provider-aware key grabber.  Pulls the constantly-
...
static void ask_fetch_random_key(void) {
    ...
    /* office48 — pull the model name from the same row.  The README
...
static void ask_settings_modal(void) {
    ...
    while (1) {
        ...
        if (!editing) {
             ...
             * bytes in one read; pre-office48 only the first was
...
#define RPG_MAP_H       64
/* office48 — 3×3 mosaic of overworlds.  The world-cell arrays cover
...
/* office48 — meta-overworld coordinate stack.  rpg_world_pos[0] is
...
/* office48 NPC layer.  rpg_npc_at[i] = 0 → no NPC; otherwise the byte
...
/* office48 wander layer.  Each animal/NPC has a current step index
...
/* office48 — when set, rpg_render_view drops origin_y to 0 and uses
...
static void rpg_init_map(const unsigned char *ruleset) {
    /* office48 — populate the 192×192 mosaic by running 9 independent
...
static void rpg_init_entities(int spawn_x, int spawn_y) {
    ...
    /* office48 — seed entities per-sub-overworld so each chunk's
...
/* office48 — generate a procedural closed-loop path of hex directions.
...
/* ── saver: rpg screensaver (office48) ────────────────────
...
/* ── bytebeat: tiny PCM synth (office48) ─────────────────
