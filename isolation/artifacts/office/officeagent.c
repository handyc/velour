...
static int run_shell(int argc, char **argv) {
    ...
    while (running) {
        ...
        body_at(2, 4, "  sheet  xpg  ask  prompt  coder  exit",
                SCREEN_W - 4);
        body_at(2, 5, "  (coder = iterative LLM code generator; prompt edits the",
                SCREEN_W - 4);
        body_at(2, 6, "   4 KB personality bank.  Alt+F / F10 opens app menus.)",
                SCREEN_W - 4);
        if (msg[0]) {
