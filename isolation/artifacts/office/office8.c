/* office8.c — Win95-style 13-app suite. Linux x86_64. No libc.
 *
 *   shell  notepad  word  mail  sheet  paint  hex  bfc  files
 *   find  calc  mines  ask   garden
 *
 * Same apps as office7.  Two display fixes:
 *   - The chrome was hard-coded to 80×24.  On a narrower terminal
 *     (anything under 80 cols) the rightmost (80 - termwidth) chars
 *     of the status row wrapped onto the next line and stayed there,
 *     because nothing else painted line 25.  All other rows had the
 *     same wrap, but the next cup() overwrote it before flush — only
 *     the final row's wrap survived.  Fix: query TIOCGWINSZ at
 *     startup via term_init() and let SCREEN_W/SCREEN_H resolve to
 *     runtime variables instead of macros.  The suite is now
 *     width-aware end-to-end with no per-callsite changes.
 *   - garden's bottom hint row called blanks(cols) after the 66-char
 *     hint string instead of blanks(cols - 66), so 66 spaces wrapped
 *     to the next line on terminals tall enough to enable chrome_bot.
 *     Fix: clamp to the leftover columns.
 */
...
static void garden_render_grid(int cursor, int cols, int rows) {
    ...
    if (chrome_bot) {
        ...
        fbs(" SPACE mark | ENTER breed | P preview | R random | "
            "S save | Q quit");
        /* Hint is 66 cols; fill the rest of the bar.  Pre-office8
         * this called blanks(cols), wrapping 66 spaces onto the row
         * below. */
        blanks(cols > 66 ? cols - 66 : 0);
    }
}
