 ...
 * situation, the active output value, and a 16×16 live preview
...
static int run_workshop(int argc, char **argv) {
    ...
    while (1) {
        ...
        char buf[160]; int bl;
        bl = 0;
        bl  = sapp(buf, bl, "GENOME WORKSHOP · ANSI-C v1.3   ");
        ...
        /* Live preview. */
        bl = 0;
        bl = sapp(buf, bl, "preview · 2 steps under current genome:\n");
        wr(1, buf, bl);
        for (int y = 0; y < WS_PREVIEW_H; y++) {
            bl = 0;
            bl = sapp(buf, bl, "  ");
    ...
    if (scmp(cmd, "officerpg") == 0) {
        ...
        if (sub_argc > 1 &&
            (scmp(sub_argv[1], "--help") == 0 ||
             scmp(sub_argv[1], "help")   == 0)) {
            static const char H[] =
                "officerpg — hex CA + L-system rpg (ANSI-C v1.4)\n"
                "\n"
        ...
        if (sub_argc > 1 &&
            (scmp(sub_argv[1], "--version") == 0 ||
             scmp(sub_argv[1], "version")   == 0)) {
            static const char V[] = "officerpg ANSI-C v1.4\n";
            wr(1, V, sizeof V - 1);
            return 0;
        }
        if (sub_argc > 1 &&
