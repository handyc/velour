# officerpg v1.8 — ANSI-C terminal hex-CA RPG
...
./officerpg seed-from-image PATH | -                     # PPM/PGM → bundle
./officerpg seed-from-mandel [--direct] [--meta] [CX CY SPAN]  # fractal → bundle
...
- v1.3 — `genome-workshop` (ev1): new `workshop` subcommand,
  ...
  x clear, W save (writes hxhnt.seed), q/ESC quit.
- v1.4 — `image-presets` (ev33): new `seed-from-image PATH | -`
  subcommand.  Reads binary PPM (P6) or PGM (P5), streams
  per-cell colour averages into a 64×64 grid, runs k-means K=4
  (8 iterations) in RGB space, sorts centroids by luminance so
  cell value 0 is the darkest, writes both `hxhnt.seed` (palette
  set from k-means centroids; existing genome retained) and
  `officerpg-state.bin` (bundle whose central panel is the
  quantised image; surrounding panels generated normally;
  entities re-derived).  Workflow: run once to plant the
  preset, then `./officerpg` → Shift+L to load the bundle.
  Bidir matrix now 20 done / 12 na for ANSI-C.
- v1.8 — `mandel-presets` (ev68): new `seed-from-mandel` subcommand.
  Samples the Mandelbrot Set at a window (cx, cy, span), runs the
  iteration in pure SSE2 doubles (no libm), then histogram-equalises
  escape times into 4 buckets — interior → rock, outer band → sand,
  mid → soil, near-boundary → water. Installs a canonical fractal
  palette (deep navy / warm gold / green / magenta).  `--direct`
  labels the bundle 'direct' (CA still cooks on cross-over in
  interactive mode).  `--meta` paints all 9 panels with one
  Mandelbrot window each, the centres drifting by half a span per
  step — walk between panels and the parameter space drifts under
  you. Iter cap auto-ramps with zoom (192 at span≥1, +64 per halving
  up to 4096).  Canon coords in `--help`: whole, seahorse, elephant,
  triple, minibrot, lightning.  Mirror of `officerpghiresev68.html`
  on the JS side.
