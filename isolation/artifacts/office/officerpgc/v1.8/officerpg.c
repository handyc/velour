...
/* v1.8: tiny no-libc atof.  Handles signed integer + fraction +
 * optional [eE][+-]?digits exponent.  No NaN/Inf/hex parsing —
 * just enough for the Mandelbrot window args (cx, cy, span). */
static double atof_(const char *s) {
    double sign = 1.0, n = 0.0;
    while (*s == ' ' || *s == '\t') s++;
    if (*s == '-')      { sign = -1.0; s++; }
    else if (*s == '+') { s++; }
    while (*s >= '0' && *s <= '9') { n = n * 10.0 + (*s - '0'); s++; }
    if (*s == '.') {
        s++;
        double frac = 1.0;
        while (*s >= '0' && *s <= '9') {
    ...
    if (*s == 'e' || *s == 'E') {
        ...
        while (*s >= '0' && *s <= '9') { eexp = eexp * 10 + (*s - '0'); s++; }
        double p = 1.0;
        for (int i = 0; i < eexp; i++) p *= 10.0;
        if (esign < 0) n /= p; else n *= p;
    }
    return sign * n;
}
...
/* ── v1.3: genome workshop — interactive 16384-rule editor ────────
 * (display fix landed in v1.5; see header below)
 *
 * Subcommand: ./officerpg workshop
 *
 * Navigates situation index 0..16383 of the active hx_seed_genome
 * (4 colours per state, 4^7 situations).  Each situation is the
 * tuple (self, n0, n1, n2, n3, n4, n5) ∈ K^7; the genome stores
 * the next-state value 0..3 for each as 2 packed bits.
 *
 * Renders a 7-swatch row (self + 6 neighbours) for the current
 * situation, the active output value, and a 32×16 live preview
 * stepped 2 ticks under the current genome from a deterministic
 * seed.  Edits trigger an immediate re-render of the preview, so
 * "this output flip changes the world like THIS" is a one-key
 * feedback loop.
 *
 * Keys:
 *   h l       situation ±1
 *   j k       situation ±16   (steps neighbour 5)
 *   H L       situation ±256  (steps neighbour 4)
 *   J K       situation ±4096 (steps self colour)
 *   g G       jump to first / last situation
 *   0 1 2 3   set output of current situation
 *   r         randomise current output
 *   x         clear current output (= 0)
 *   W         write hxhnt.seed (persist edits)
 *   q ESC     quit (warns if unsaved)
 *
 * v1.5 fix: workshop's preview row was writing ~320 bytes per row
 * into a 160-byte stack buffer.  sapp() is unbounded, so each ANSI
 * "\x1b[48;5;NNNm " sequence past byte 160 trampled adjacent stack
 * (idx, dirty, n[6], values[7], saved RBP).  Symptom: scrambled
 * cursor positions, partial rows, garbage cells, occasional crash
 * on quit when the trashed return address jumped to nowhere.
 * Fix: bump the workshop's local buffer to 2048 bytes, make each
 * cell two character columns wide so the hex grid actually looks
 * hex-spaced, and add a row-by-row reset to keep terminals from
 * trailing the last cell's color past the newline.
 *
 * v1.7 fix: v1.6's "deferred \x1b[2J at init + fbflush at end of
 * frame" combo flushed the *clear* AFTER every paint.  The framebuffer
 * carried the pending [2J across frames; each fbflush() at end-of-
 * frame wrapped that clear in DECSET 2026 and sent it.  Result: the
 * workshop painted (visible for milliseconds) then the [2J wiped
 * everything, then the next iteration painted again.  User perceived
 * this as "scrolls off, screen mostly blank, only visible if I
 * scroll back."  Fix: drop the deferred fbs("\x1b[2J") at workshop
 * init AND the fbflush() at end of frame; clear once on entry via
 * a direct wr() (so it doesn't sit in fb waiting to be flushed at
 * the wrong time), and never touch fb during the loop — workshop
 * uses wr() directly for every render byte.
 *
 * v1.6 (kept layout): v1.5 rendered 26-29 lines of content into a
 * 24-row terminal — the bottom lines pushed the top off-screen,
 * so on a default 80x24 the user only saw the footer with the
 * actual workshop hidden in scrollback.  Fix: use absolute
 * cursor positioning (\x1b[Y;XH) for every line and lay the
 * workshop out to fit in 24 rows exactly:
 *   Row 1     header (idx + dirty flag)
 *   Row 2     swatch labels  s  n0..n5
 *   Row 3     swatch colour blocks
 *   Row 4     swatch values (digits)
 *   Row 5     Output: N  swatch  [press 0/1/2/3 to set]
 *   Row 6     "preview · 32x16 · 2 steps under current genome:"
 *   Rows 7-22 16 preview rows (16 high)
 *   Row 23    Nav: hl jk HL JK g/G first/last
 *   Row 24    Edit: 0/1/2/3 r x W save · q ESC quit
 * Last line ends without a trailing newline so the cursor parks
 * at row 24 col EOL — the screen never scrolls regardless of
 * terminal-quirk flushing behaviour.  WS_PREVIEW_H stays 16. */
...
static void ws_step_preview(unsigned long seed) {
    /* Deterministic init from seed → 2 steps under current genome
     * → leave result in ws_preview_a so the renderer can read it. */
    unsigned long s = seed | 1UL;
    for (int i = 0; i < WS_PREVIEW_W * WS_PREVIEW_H; i++) {
        s = s * 6364136223846793005UL + 1442695040888963407UL;
        ws_preview_a[i] = (unsigned char)((s >> 30) & 3);
    }
    /* Tiny 32×16 hex stepper inline — same logic as hx_step_grid
     * but smaller buffer; reused HX_DY/DXE/DXO offsets. */
    for (int t = 0; t < 2; t++) {
        ...
        unsigned char *tmp = (unsigned char *)ws_preview_a;
        mcpy(tmp, ws_preview_b, WS_PREVIEW_W * WS_PREVIEW_H);
    }
}
...
/* v1.6: emit "go to row R, col 1" sequence at the start of a row. */
...
static int run_workshop(int argc, char **argv) {
    ...
    int dirty = 0;
    term_raw();
    /* v1.5: buffer sized for the widest line we ever build — the
     * preview row is 32 cells × ~12 bytes ≈ 400 bytes, plus header,
     * trailing reset, newline.  2048 leaves head-room. */
    char buf[2048];
    /* v1.7: clear ONCE on entry via a DIRECT wr() — using fbs() here
     * deferred the clear into the framebuffer, where the per-frame
     * fbflush() below was sending it AFTER every paint and wiping
     * the workshop (v1.6's bug). */
    wr(1, "\x1b[2J\x1b[H", 7);
    while (1) {
        ws_step_preview(0xa5a5a5a5UL);   /* fixed seed for stable visual diff */
        ...
        bl = 0;
        bl = ws_goto(buf, bl, 1);
        bl = sapp(buf, bl, "\x1b[K");   /* erase to EOL */
        bl = sapp(buf, bl, "GENOME WORKSHOP · ANSI-C v1.7   ");
        bl += utoa((unsigned)idx, buf + bl);
        bl = sapp(buf, bl, " / 16383");
        if (dirty) bl = sapp(buf, bl, "  *unsaved*");

        /* 7 colour swatches: self + n0..n5. */
        int self_v;
        int n[6];
        ...
        /* Flush so far so big preview-row writes get their own buffers. */
        wr(1, buf, bl);

        /* Rows 7..22 — preview, 16 rows of 32 cells each. */
        for (int y = 0; y < WS_PREVIEW_H; y++) {
            bl = 0;
        ...
        /* Row 24 — Edit + quit (last row, no trailing newline). */
        bl = ws_goto(buf, bl, 24);
        bl = sapp(buf, bl,
            "\x1b[KEdit: 0/1/2/3 r x W save · q ESC quit");
        wr(1, buf, bl);
        /* v1.7: NO fbflush here — the workshop renders entirely via
         * direct wr() above, nothing pending in fb.  v1.6 left a
         * stale "\x1b[2J" in fb from init, so each fbflush at this
         * spot wiped the screen right after we painted it. */
...
/* ── v1.4: image-presets — PPM/PGM → terrain seed (port of ev33) ──
 *
 * Subcommand: ./officerpg seed-from-image PATH | -
 *
 * Reads a binary PPM (P6, RGB) or PGM (P5, grayscale) from path or
 * stdin, downsamples to 64×64 by per-cell color averaging, runs
 * k-means K=4 in RGB space (8 iterations) to derive a 4-colour
 * palette, sorts the palette by luminance so cell value 0 is the
 * darkest, and writes:
 *
 *   - hxhnt.seed   palette set from k-means centroids; genome
 *                  unchanged (whatever was loaded at startup)
 *   - officerpg-state.bin  full bundle whose central 64×64 panel
 *                          is the quantised image; surrounding
 *                          panels regenerated normally; entities
 *                          re-derived from the image-driven
 *                          terrain
 *
 * Workflow: run this once to plant the preset, then `./officerpg`
 * → Shift+L to load the bundle.  The palette persists across
 * subsequent launches (read by hx_active_init from hxhnt.seed). */
static int read_ppm_token(int fd, int *out) {
    char c;
    *out = 0;
    int started = 0;
    while (1) {
        long n = rd(fd, &c, 1);
        if (n != 1) return started ? 0 : -1;
        if (c == '#') {
            while (rd(fd, &c, 1) == 1 && c != '\n') {}
            continue;
        }
        ...
        if (started) return 0;   /* whitespace after digits → done */
    }
}

/* Per-cell accumulators for streaming downsample.  64×64 = 4096
 * target cells; each receives many source pixels (for typical
 * images ≥ 256×256, dozens to thousands of pixels per cell). */
static unsigned long ip_sum_r[64][64];
static unsigned long ip_sum_g[64][64];
static unsigned long ip_sum_b[64][64];
static unsigned int  ip_cnt  [64][64];
static unsigned char ip_avg  [64][64][3];
static unsigned char ip_assign[64][64];

static int run_seed_from_image(int argc, char **argv) {
    if (argc < 2) {
        static const char U[] =
            "usage: ./officerpg seed-from-image PATH | -\n"
            "  PATH is a binary PPM (P6) or PGM (P5) file.\n"
            "  -    reads from stdin.\n";
        wr(2, U, sizeof U - 1);
        return 2;
    }
    ...
    char magic[2];
    if (rd(fd, magic, 2) != 2) {
        wr(2, "short read on magic\n", 20);
        if (fd != 0) cl(fd);
        return 1;
    }
    if (magic[0] != 'P' || (magic[1] != '5' && magic[1] != '6')) {
        wr(2, "not a binary PPM (P6) or PGM (P5)\n", 34);
        if (fd != 0) cl(fd);
        return 1;
    }
    int chan = (magic[1] == '6') ? 3 : 1;
    int w, h, max;
    if (read_ppm_token(fd, &w) < 0
     || read_ppm_token(fd, &h) < 0
     || read_ppm_token(fd, &max) < 0
     || w <= 0 || h <= 0 || max <= 0) {
        wr(2, "bad header\n", 11);
        if (fd != 0) cl(fd);
        return 1;
    }
    /* Stream pixels, accumulate per-target-cell sums. */
    mset(ip_sum_r, 0, sizeof ip_sum_r);
    mset(ip_sum_g, 0, sizeof ip_sum_g);
    mset(ip_sum_b, 0, sizeof ip_sum_b);
    mset(ip_cnt,   0, sizeof ip_cnt);
    for (int sy = 0; sy < h; sy++) {
        for (int sx = 0; sx < w; sx++) {
            unsigned char rgb[3];
            long n = rd(fd, rgb, (size_t)chan);
            if (n != chan) goto eof;
            unsigned long r = rgb[0];
            unsigned long g = (chan == 3) ? rgb[1] : r;
            unsigned long b = (chan == 3) ? rgb[2] : r;
            int cx = (sx * 64) / w;
            int cy = (sy * 64) / h;
            if (cx > 63) cx = 63;
            if (cy > 63) cy = 63;
...
eof:
    ...
    /* Compute per-cell averaged RGB. */
    for (int cy = 0; cy < 64; cy++) {
        for (int cx = 0; cx < 64; cx++) {
            unsigned int n = ip_cnt[cy][cx];
            if (n == 0) {
                ip_avg[cy][cx][0] = 0;
                ip_avg[cy][cx][1] = 0;
                ip_avg[cy][cx][2] = 0;
            } else {
                ip_avg[cy][cx][0] = (unsigned char)(ip_sum_r[cy][cx] / n);
                ip_avg[cy][cx][1] = (unsigned char)(ip_sum_g[cy][cx] / n);
                ip_avg[cy][cx][2] = (unsigned char)(ip_sum_b[cy][cx] / n);
            }
        }
    }

    /* K-means K=4: init at 4 corners of a brightness-spread cube. */
    unsigned char cent[4][3] = {
        {  30,  30,  30},
        { 200, 100, 100},
        { 100, 200, 100},
        { 100, 100, 200},
    ...
    for (int it = 0; it < 8; it++) {
        for (int cy = 0; cy < 64; cy++) {
            for (int cx = 0; cx < 64; cx++) {
                ...
                for (int k = 0; k < 4; k++) {
                    long dr = (long)ip_avg[cy][cx][0] - cent[k][0];
                    long dg = (long)ip_avg[cy][cx][1] - cent[k][1];
                    long db = (long)ip_avg[cy][cx][2] - cent[k][2];
                    long d = dr*dr + dg*dg + db*db;
                    if (d < bestd) { bestd = d; best = k; }
                }
                ip_assign[cy][cx] = (unsigned char)best;
            }
        }
        unsigned long sr[4] = {0, 0, 0, 0};
        unsigned long sg[4] = {0, 0, 0, 0};
        unsigned long sb[4] = {0, 0, 0, 0};
        unsigned int  cn[4] = {0, 0, 0, 0};
        for (int cy = 0; cy < 64; cy++) {
            for (int cx = 0; cx < 64; cx++) {
                int k = ip_assign[cy][cx];
                sr[k] += ip_avg[cy][cx][0];
                sg[k] += ip_avg[cy][cx][1];
                sb[k] += ip_avg[cy][cx][2];
                cn[k]++;
        ...
        for (int k = 0; k < 4; k++) {
            if (cn[k] > 0) {
                cent[k][0] = (unsigned char)(sr[k] / cn[k]);
                cent[k][1] = (unsigned char)(sg[k] / cn[k]);
                cent[k][2] = (unsigned char)(sb[k] / cn[k]);
            }
        }
    }

    /* Sort centroids by luminance (R+G+B), so value 0 is darkest.
     * Bubble sort across 4 elements is fine. */
    int order[4] = { 0, 1, 2, 3 };
    for (int i = 0; i < 3; i++) {
        for (int j = 0; j < 3 - i; j++) {
            int bj  = (int)cent[order[j  ]][0] + cent[order[j  ]][1] + cent[order[j  ]][2];
            int bj1 = (int)cent[order[j+1]][0] + cent[order[j+1]][1] + cent[order[j+1]][2];
            if (bj > bj1) { int t = order[j]; order[j] = order[j+1]; order[j+1] = t; }
        }
    }
    int remap[4];
    ...
    for (int cy = 0; cy < 64; cy++) {
        for (int cx = 0; cx < 64; cx++) {
            ip_assign[cy][cx] = (unsigned char)remap[ip_assign[cy][cx]];
        }
    }
    unsigned char ordered[4][3];
    for (int i = 0; i < 4; i++) {
        ordered[i][0] = cent[order[i]][0];
        ordered[i][1] = cent[order[i]][1];
        ordered[i][2] = cent[order[i]][2];
    }

    /* Bootstrap world + override central panel terrain. */
    hx_active_init();
    for (int i = 0; i < 4; i++) {
        hx_seed_pal[i] = (unsigned char)rpg_rgb_to_xterm256(
            ordered[i][0], ordered[i][1], ordered[i][2]);
    }
    rpg_sprites_init();
    mset(rpg_world_pos, 0, sizeof rpg_world_pos);
    int spawn_x = RPG_TILE_W / 2;
    int spawn_y = RPG_TILE_H / 2;
    rpg_load_overworld(spawn_x, spawn_y);
    /* Overwrite central 64×64 panel with the image-quantised cells. */
    int center_sx = RPG_MAP_W;     /* world_x = 64..127 */
    int center_sy = RPG_MAP_H;
    for (int cy = 0; cy < RPG_MAP_H; cy++) {
        for (int cx = 0; cx < RPG_MAP_W; cx++) {
            int wx = center_sx + cx;
            int wy = center_sy + cy;
            rpg_map[wy * RPG_TILE_W + wx] = ip_assign[cy][cx];
        }
    }
    /* Refresh entities so categories match the new terrain. */
    rpg_init_entities(spawn_x, spawn_y);
    rpg_player_init();
    rpg_genome_live_cache = -1;
    mset(rpg_cell_done, 0, sizeof rpg_cell_done);
    /* Persist palette + bundle. */
    hx_write_seed(HX_SEED_FILE, hx_seed_pal, hx_seed_genome);
    rpg_save_bundle(NULL);
    /* Report. */
    char rep[200]; int rl = 0;
    rl  = sapp(rep, rl, "image-preset · ");
    rl += utoa((unsigned)w, rep + rl);
    rep[rl++] = 'x';
    rl += utoa((unsigned)h, rep + rl);
    rl  = sapp(rep, rl, " → 64x64 cells · palette={");
    for (int i = 0; i < 4; i++) {
        rep[rl++] = '(';
        rl += utoa(ordered[i][0], rep + rl); rep[rl++] = ',';
        rl += utoa(ordered[i][1], rep + rl); rep[rl++] = ',';
        rl += utoa(ordered[i][2], rep + rl); rep[rl++] = ')';
        if (i < 3) rep[rl++] = ' ';
    }
    rl  = sapp(rep, rl, "}\nwrote hxhnt.seed + officerpg-state.bin\n");
    wr(1, rep, rl);
    return 0;
}


/* ── v1.8: seed-from-mandel ──────────────────────────────────
 *
 * Same pipeline as seed-from-image but the source is a Mandelbrot
 * window instead of a PPM/PGM file.  Sample the set at (cx, cy) over
 * a horizontal span, count iterations per cell, histogram-equalise
 * escape times into 4 buckets, and drop the result into the central
 * panel of the overworld (--meta writes all 9 panels using a
 * different window per panel).
 *
 * Subcommand:
 *   ./officerpg seed-from-mandel                       Whole set
 *   ./officerpg seed-from-mandel CX CY SPAN            custom window
 *   ./officerpg seed-from-mandel --direct ...          mark "direct" in
 *                                                      report (CA still
 *                                                      runs once in the
 *                                                      central panel; the
 *                                                      rest of the world
 ...
 *                                                      the normal CA on
 *                                                      cross-over)
 *   ./officerpg seed-from-mandel --meta [SPAN]         9 panels, one
 *                                                      Mandelbrot window
 *                                                      each
 *
 * The set is iterated with cap auto-ramped by zoom (mirroring the
 * officemandel rule: 192 at span≥1, +64 every halving down to 4096).
 * Bailout < 4.0; no smooth coloring (avoids the math library). */
static int mandel_iter_cap_for(double span) {
    int it = 192;
    double s = span;
...
/* Compute escape iter for one point.  Returns -1 if the point is in
 * the set (didn't escape within cap iterations), else 0..cap-1. */
static int mandel_iter_at(double cx, double cy, int cap) {
    double zx = 0, zy = 0, x2 = 0, y2 = 0;
    int i;
    for (i = 0; i < cap && x2 + y2 < 4.0; i++) {
        zy = 2.0 * zx * zy + cy;
        zx = x2 - y2 + cx;
        x2 = zx * zx;
        y2 = zy * zy;
...
/* Sample a 64×64 Mandelbrot window into ip_assign[][] using histogram
 * equalisation across escapees so the four terrain classes always end
 * up with roughly equal frequency.  Bucket 0 = interior. */
static void mandel_quantise_64x64(double cx, double cy, double span, int cap) {
    int raw[64][64];
    int sx, sy;
    double s  = span / 64.0;                 /* square pixels in C-plane */
    double ox = cx - s * 64.0 * 0.5;
    double oy = cy - s * 64.0 * 0.5;
    for (sy = 0; sy < 64; sy++) {
        double py = oy + sy * s;
        for (sx = 0; sx < 64; sx++) {
            raw[sy][sx] = mandel_iter_at(ox + sx * s, py, cap);
        }
    }
    /* Build histogram of escapees to find ⅓ and ⅔ tertile boundaries. */
    int hist[4097];                          /* cap can grow to 4096 */
    ...
    for (sy = 0; sy < 64; sy++) {
        for (sx = 0; sx < 64; sx++) {
            int v = raw[sy][sx];
            if (v >= 0 && v <= cap) { hist[v]++; escapees++; }
        }
    }
    int t1 = -1, t2 = -1;
    if (escapees > 0) {
        int target1 = (escapees + 2) / 3;
        int target2 = (2 * escapees + 1) / 3;
        int cum = 0;
        for (int v = 0; v <= cap; v++) {
            cum += hist[v];
            if (t1 < 0 && cum >= target1) t1 = v;
            if (t2 < 0 && cum >= target2) { t2 = v; break; }
        }
    }
    if (t1 < 0) t1 = cap;
    if (t2 < 0) t2 = cap;
    for (sy = 0; sy < 64; sy++) {
        for (sx = 0; sx < 64; sx++) {
            int v = raw[sy][sx];
            unsigned char b;
            if (v < 0)        b = 0;        /* interior   → rock     */
            else if (v <= t1) b = 1;        /* outer band → sand     */
            else if (v <= t2) b = 2;        /* mid        → soil     */
            else              b = 3;        /* near edge  → water    */
            ip_assign[sy][sx] = b;
        }
    }
}

static int run_seed_from_mandel(int argc, char **argv) {
    /* Defaults — whole set. */
    double cx = -0.5, cy = 0.0, span = 3.0;
    int direct = 0;
    int meta = 0;
    /* Parse: [--direct] [--meta] [cx cy span] */
    int positional[4] = {0, 0, 0, 0};
    int npos = 0;
    for (int a = 1; a < argc; a++) {
        if      (scmp(argv[a], "--direct") == 0) direct = 1;
        else if (scmp(argv[a], "--meta")   == 0) meta   = 1;
        else if (scmp(argv[a], "--help")   == 0 ||
                 scmp(argv[a], "-h")       == 0) {
            static const char U[] =
                "usage: ./officerpg seed-from-mandel [--direct] [--meta] [CX CY SPAN]\n"
                "  CX, CY    centre of the Mandelbrot window (default -0.5, 0)\n"
                "  SPAN      horizontal extent in the complex plane (default 3.0)\n"
                "  --direct  label the bundle 'direct' (no extra CA cooking on save)\n"
                "  --meta    write all 9 panels, each with a different window\n"
                "presets (paste the numbers as CX CY SPAN):\n"
                "  whole         -0.5      0          3.0\n"
                "  seahorse      -0.745    0.105      0.04\n"
                "  elephant       0.275    0.006      0.04\n"
                "  triple        -0.088    0.654      0.014\n"
                "  minibrot      -1.7494   0          0.02\n"
                "  lightning     -1.255    0.382      0.025\n";
            wr(2, U, sizeof U - 1);
            return 0;
        } else if (npos < 4) {
            positional[npos++] = a;
    ...
    if (npos >= 3) {
        cx   = atof_(argv[positional[0]]);
        cy   = atof_(argv[positional[1]]);
        span = atof_(argv[positional[2]]);
        if (!(span > 0.0)) span = 3.0;
    } else if (npos == 1 && meta) {
        /* `--meta SPAN` form — use SPAN as per-panel window size,
         * keep cx/cy at defaults. */
        span = atof_(argv[positional[0]]);
        if (!(span > 0.0)) span = 0.6;
    }
    int cap = mandel_iter_cap_for(span);

    /* Bootstrap world. */
    hx_active_init();
    /* Install canonical fractal palette (matches MANDEL_PALETTE in
     * ev68): deep navy / warm gold / green / magenta. */
    static const unsigned char mandel_rgb[4][3] = {
        { 18,  20,  46},
        {232, 188,  68},
        { 64, 170,  86},
        {196,  84, 196},
    ...
    for (int i = 0; i < 4; i++) {
        hx_seed_pal[i] = (unsigned char)rpg_rgb_to_xterm256(
            mandel_rgb[i][0], mandel_rgb[i][1], mandel_rgb[i][2]);
    }
    rpg_sprites_init();
    mset(rpg_world_pos, 0, sizeof rpg_world_pos);
    int spawn_x = RPG_TILE_W / 2;
    ...
    if (meta) {
        /* 9 panels, one window each.  Drift base (cx, cy) by ±span
         * along both axes — central panel = (cx, cy), neighbours = a
         * step in each direction. */
        for (int pry = 0; pry < 3; pry++) {
            int dy = pry - 1;
            for (int prx = 0; prx < 3; prx++) {
                int dx = prx - 1;
                double pcx = cx + dx * span * 0.5;
                double pcy = cy + dy * span * 0.5;
                int pcap = mandel_iter_cap_for(span);
                mandel_quantise_64x64(pcx, pcy, span, pcap);
                int base_sx = prx * RPG_MAP_W;
                int base_sy = pry * RPG_MAP_H;
                for (int cyl = 0; cyl < RPG_MAP_H; cyl++) {
                    for (int cxl = 0; cxl < RPG_MAP_W; cxl++) {
                        rpg_map[(base_sy + cyl) * RPG_TILE_W + base_sx + cxl] =
                            ip_assign[cyl][cxl];
                    }
                }
            }
        ...
        mandel_quantise_64x64(cx, cy, span, cap);
        int center_sx = RPG_MAP_W;
        int center_sy = RPG_MAP_H;
        for (int cyl = 0; cyl < RPG_MAP_H; cyl++) {
            for (int cxl = 0; cxl < RPG_MAP_W; cxl++) {
                rpg_map[(center_sy + cyl) * RPG_TILE_W + center_sx + cxl] =
                    ip_assign[cyl][cxl];
            }
        }
    }

    /* Refresh entities + persist palette + bundle. */
    rpg_init_entities(spawn_x, spawn_y);
    rpg_player_init();
    rpg_genome_live_cache = -1;
    mset(rpg_cell_done, 0, sizeof rpg_cell_done);
    ...
    /* Report — print "cx=… cy=… span=… iter=… mode=…".  Floats: emit
     * sign + integer part + 6 fractional digits via integer-only
     * scaling, since no libc means no %f.  Span is reported the same
     * way so deep-zoom values survive the round-trip. */
    char rep[300]; int rl = 0;
    rl  = sapp(rep, rl, "mandel · cx=");
    /* fprint helper, inline: writes sign + intpart.6frac to rep+rl */
    {
        double vals[3] = { cx, cy, span };
        const char *labels[3] = { NULL, " cy=", " span=" };
        for (int v = 0; v < 3; v++) {
            if (labels[v]) rl  = sapp(rep, rl, labels[v]);
            double x = vals[v];
            if (x < 0) { rep[rl++] = '-'; x = -x; }
            unsigned long ip = (unsigned long)x;
            rl += utoa((unsigned)ip, rep + rl);
            rep[rl++] = '.';
    ...
    rl  = sapp(rep, rl, " iter=");
    rl += utoa((unsigned)cap, rep + rl);
    rl  = sapp(rep, rl, " mode=");
    if (meta)        rl = sapp(rep, rl, "meta");
    else if (direct) rl = sapp(rep, rl, "direct");
    else             rl = sapp(rep, rl, "seed");
    rl  = sapp(rep, rl, "\nwrote hxhnt.seed + officerpg-state.bin\n");
    wr(1, rep, rl);
    return 0;
}
    ...
    if (scmp(cmd, "officerpg") == 0) {
        ...
        if (sub_argc > 1 &&
            (scmp(sub_argv[1], "--help") == 0 ||
             scmp(sub_argv[1], "help")   == 0)) {
            static const char H[] =
                "officerpg — hex CA + L-system rpg (ANSI-C v1.8)\n"
                "\n"
                "  ./officerpg              run interactive rpg\n"
                "  ./officerpg saver        run journey-mode screensaver\n"
                "  ./officerpg workshop     interactive 16384-rule editor\n"
                "  ./officerpg seed-from-image PATH | -\n"
                "                           PPM/PGM → palette + terrain bundle\n"
                "  ./officerpg seed-from-mandel [--direct] [--meta] [CX CY SPAN]\n"
                "                           Mandelbrot → palette + terrain bundle\n"
        ...
        if (sub_argc > 1 &&
            (scmp(sub_argv[1], "--version") == 0 ||
             scmp(sub_argv[1], "version")   == 0)) {
            static const char V[] = "officerpg ANSI-C v1.8\n";
            wr(1, V, sizeof V - 1);
            return 0;
        }
        if (sub_argc > 1 &&
        ...
        /* v1.3: genome workshop — interactive 16384-rule editor. */
        if (sub_argc > 1 && scmp(sub_argv[1], "workshop") == 0)
            return run_workshop(sub_argc - 1, sub_argv + 1);
        /* v1.4: image preset — PPM/PGM → terrain seed bundle. */
        if (sub_argc > 1 && scmp(sub_argv[1], "seed-from-image") == 0)
            return run_seed_from_image(sub_argc - 1, sub_argv + 1);
        /* v1.8: Mandelbrot preset — fractal window → terrain seed bundle. */
        if (sub_argc > 1 && scmp(sub_argv[1], "seed-from-mandel") == 0)
            return run_seed_from_mandel(sub_argc - 1, sub_argv + 1);
