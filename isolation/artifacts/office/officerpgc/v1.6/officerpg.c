 ...
 *
 * v1.6 fix: v1.5 still rendered 26-29 lines of content into a
...
static int run_workshop(int argc, char **argv) {
    ...
    char buf[2048];
    /* v1.6: clear screen ONCE — subsequent frames overwrite each row
     * via absolute cursor positioning so we never trigger a scroll. */
    fbs("\x1b[2J");
    while (1) {
        ...
        bl = sapp(buf, bl, "GENOME WORKSHOP · ANSI-C v1.6   ");
    ...
    if (scmp(cmd, "officerpg") == 0) {
        ...
        if (sub_argc > 1 &&
            (scmp(sub_argv[1], "--help") == 0 ||
             scmp(sub_argv[1], "help")   == 0)) {
            static const char H[] =
                "officerpg — hex CA + L-system rpg (ANSI-C v1.6)\n"
                "\n"
        ...
        if (sub_argc > 1 &&
            (scmp(sub_argv[1], "--version") == 0 ||
             scmp(sub_argv[1], "version")   == 0)) {
            static const char V[] = "officerpg ANSI-C v1.6\n";
            wr(1, V, sizeof V - 1);
            return 0;
        }
        if (sub_argc > 1 &&
