...
static int run_workshop(int argc, char **argv) {
    ...
    while (1) {
        ...
        int bl;
        bl = 0;
        bl  = sapp(buf, bl, "GENOME WORKSHOP · ANSI-C v1.5   ");
        ...
        /* Live preview — each cell is 2 columns wide so the 32×16
         * grid renders square-ish.  Reset before newline so the
         * last colour doesn't bleed into the terminal margin. */
        bl = 0;
        bl = sapp(buf, bl, "preview · 32x16 · 2 steps under current genome:\n");
        wr(1, buf, bl);
        for (int y = 0; y < WS_PREVIEW_H; y++) {
            bl = 0;
            bl = sapp(buf, bl, "  ");
        ...
        bl = 0;
        bl = sapp(buf, bl,
            "\nNav: hl +-1 · jk +-16 · HL +-256 · JK +-4096 · g/G first/last\n"
            "Edit: 0/1/2/3 set · r random · x clear · W save (hxhnt.seed)\n"
            "                                                        q ESC quit\n");
        wr(1, buf, bl);
        fbflush();
    ...
    if (scmp(cmd, "officerpg") == 0) {
        ...
        if (sub_argc > 1 &&
            (scmp(sub_argv[1], "--help") == 0 ||
             scmp(sub_argv[1], "help")   == 0)) {
            static const char H[] =
                "officerpg — hex CA + L-system rpg (ANSI-C v1.5)\n"
                "\n"
        ...
        if (sub_argc > 1 &&
            (scmp(sub_argv[1], "--version") == 0 ||
             scmp(sub_argv[1], "version")   == 0)) {
            static const char V[] = "officerpg ANSI-C v1.5\n";
            wr(1, V, sizeof V - 1);
            return 0;
        }
        if (sub_argc > 1 &&
