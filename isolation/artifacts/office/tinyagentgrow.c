/* tinyagentgrow.c -- tinyagentjail + capability-tail accumulation.
 *
 * Forked from tinyagentjail 2026-05-11 (39.8 KB Phase-4 baseline).
 * Carries forward the full recursive sub-agent spawning + decompose-
 * or-answer + leaf compile-verify + parent synthesis pipeline, and
 * adds a "grow" subcommand family that extends the running binary
 * with new capability tails using the same magic-bytes pattern used
...
/* Look up a canon template by name.  Returns NULL if not present. */
...
/* Walk self's tail chain and find the most-recent type='P' tail with
 * the given name.  Returns 1 on hit, writing payload pointer + length
 * (into tg_buf — caller must have already loaded self with size B). */
...
/* Right-pad `s` to `width` columns on stdout (truncates if longer). */
static void tg_tmpl_pad(const char *s, int slen_in, int width) {
    wr(1, s, slen_in);
    for (int i = slen_in; i < width; i++) wr(1, " ", 1);
}

/* `tmpl list` — print canon + grown, one line each.  Format:
 *     canon  <name>     <bytes> B  <tag>
 *     grown  <name>     <bytes> B  <task or "(no task)"> */
...
/* `tmpl cat <name>` / `tmpl get <name>` — print template body.  Canon
 * first, then grown.  Returns 1 if name missing from both. */
...
/* `tmpl add <name> <file> [task...]` — append <file>'s contents as a
 * v2 tail with type='P'.  Refuses to shadow a canon name.  Optional
 * task description joins remaining argv (e.g. "TUI menu for foo"). */
...
/* `tmpl rm <name>` — remove a type='P' tail by name.  Won't touch
 * canon (immutable, in .rodata) and won't clip 'A'/'G'/'S'/'T' tails
 * with a name collision — same write-new-gen pattern as grow rm but
 * filtered. */
