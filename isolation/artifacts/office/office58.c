/* office58.c -- Win95-style 11-app suite. Linux x86_64. No libc.
 *
 *   shell  notepad  sheet  hex  files  calc
 *   ask   garden  hxhnt  rpg  lsys  saver
 *
 * office58 — scientific calc + 16 binary logic ops in sheet.
 *
 ...
 * Applied bitwise across all 32 bits of a and b. */
static int binlog(int a, int b, int tt) {
    int r = 0;
    if (tt & 1) r |= ~a & ~b;
    if (tt & 2) r |= ~a & b;
    if (tt & 4) r |= a & ~b;
...
static int ipow_(int base, int exp) {
    if (exp < 0) return 0;
    int r = 1;
    while (exp > 0) {
        if (exp & 1) r *= base;
...
static int feval_pow(int depth);
...
static int feval_term(int depth) {
    int v = feval_pow(depth);
    while (1) {
        fskip_ws();
        if (*fp == '*') { fp++; v *= feval_pow(depth); }
        else if (*fp == '/') { fp++; int d = feval_pow(depth); v = d ? v / d : 0; }
        else if (*fp == '%') { fp++; int d = feval_pow(depth); v = d ? v % d : 0; }
        else break;
    }
    return v;
...
/* Common: write an int into cell[2][r][c] truncated to 15 chars. */
static void sheet_put_int(int r, int c, long v) {
    char tmp[16];
    int  tn = itoa_((int)v, tmp);
    if (tn > 15) tn = 15;
    ...
    for (int i = 0; i < ar; i++) {
        for (int j = 0; j < bc; j++) {
            ...
            for (int k = 0; k < inner; k++) {
                int aV = (k < ac) ? read_int_in_sheet(0, i, k) : 0;
                int bV = (k < br) ? read_int_in_sheet(1, k, j) : 0;
                acc += (long)aV * (long)bV;
            }
...
static void sheet_kronecker(void) {
    ...
    for (int i = 0; i < ar; i++) {
        for (int j = 0; j < ac; j++) {
            int aV = read_int_in_sheet(0, i, j);
            for (int p = 0; p < br; p++) {
                int rr = i * br + p;
                if (rr >= useR) break;
                for (int q = 0; q < bc; q++) {
                    int cc = j * bc + q;
                    if (cc >= useC) break;
                    int bV = read_int_in_sheet(1, p, q);
                    sheet_put_int(rr, cc, (long)aV * (long)bV);
                }
            }
        }
    }
...
static int run_sheet(int argc, char **argv) {
    ...
    while (1) {
        ...
        const char *h = editing
            ...
            : "  arrows|e edit|tab/1-3 sheet|M=A.B C|K=A(x)B C|s save|q back";
