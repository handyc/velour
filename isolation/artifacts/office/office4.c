/* office4.c — Win95-style 12-app suite. Linux x86_64. No libc.
 *
 *   shell  notepad  word  mail  sheet  paint  hex  bfc  files
 *   find  calc  mines
 *
 * Adds on top of office3.c:
 *   - real top menu: Alt+F/E/V/H (DOS mnemonics) or F10 activates;
 *     arrows navigate, Enter selects, Esc cancels. Mnemonic letters
 *     are underlined in the menu bar (SGR 4 / 24).
 *   - cross-app clipboard: ^X cut, ^C copy, ^V paste. notepad/word
 *     copy/cut the current line; sheet copies the current cell; hex
 *     copies the 16-byte row. Paste inserts at the cursor in any.
 *   - find: grep across files in cwd; Enter on a hit opens it in
 *     notepad with the cursor on that line.
 *   - calc: single-line expression input that reuses the sheet's
 *     formula evaluator. Paste-friendly (^V from a sheet cell).
 *   - mines: 16x16 Minesweeper inside the body chrome. Arrows +
 *     space reveal + f flag + r reset.
 */
 ...
 * user can see which Alt+letter opens which menu — this matches
 * Borland/Turbo and Win95 conventions. */
 ...
 * or 0 if they cancelled with Esc. The caller treats most actions
 * as if the user had pressed the corresponding hotkey. */
static int menu_run(const MS *m, int start) {
    ...
    int mi = start;
    int tries = 0;
    while (n[mi] == 0) { mi = (mi + 1) % 4; if (++tries > 4) return 0; }
    int sel = 0;
    while (1) {
        menu_bar(mi);
        int x = 1;
        for (int j = 0; j < mi; j++) x += slen(names[j]) + 3;
        int max_w = 0;
        ...
        if (kn >= 3 && k[0] == 0x1b && k[1] == '[') {
            ...
            case 'C': do { mi = (mi + 1) % 4; } while (n[mi] == 0);
                      sel = 0; break;
            case 'D': do { mi = (mi - 1 + 4) % 4; } while (n[mi] == 0);
                      sel = 0; break;
            }
            continue;
        }
