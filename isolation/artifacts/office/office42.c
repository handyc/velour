/* office42.c — Win95-style 17-app suite. Linux x86_64. No libc.
 *
 *   shell  notepad  word  mail  sheet  paint  hex  bfc  files
 *   find  calc  mines  ask   garden  hxhnt  rpg  lsys  bytebeat
 *
 * office42 polishes the Ask app's settings modal:
 ...
 *     into /tmp/office42_keys.txt, scan for "sk-" tokens, and
 ...
 * 8000 Hz u8 mono into /tmp/office42_bb.raw and pipe to aplay via
...
/* office42 — preseed the API key with a random `sk-…` token from
...
static void ask_settings_modal(void) {
    ...
    while (1) {
        ...
        if (!editing) {
             ...
             * bytes in one read; pre-office42 only the first was
...
#define RPG_MAP_H       64
/* office42 — 3×3 mosaic of overworlds.  The world-cell arrays cover
...
/* office42 — meta-overworld coordinate stack.  rpg_world_pos[0] is
...
/* office42 NPC layer.  rpg_npc_at[i] = 0 → no NPC; otherwise the byte
...
/* office42 wander layer.  Each animal/NPC has a current step index
...
static void rpg_init_map(const unsigned char *ruleset) {
    /* office42 — populate the 192×192 mosaic by running 9 independent
...
static void rpg_init_entities(int spawn_x, int spawn_y) {
    ...
    /* office42 — seed entities per-sub-overworld so each chunk's
...
/* office42 — generate a procedural closed-loop path of hex directions.
...
/* ── bytebeat: tiny PCM synth (office42) ─────────────────
